...
class AreaTool:
    def __init__(self, alg):
        ...
        if alg != None:
            chk.check_option(alg, [CfgValue.CryptoAlg.ECC, CfgValue.CryptoAlg.SM], 'algorithm')
        if alg == CfgValue.CryptoAlg.ECC:
            self._asym_alg     = 'ECC'
            self._digest_alg   = 'SHA256'
            self._enc_ctr_alg  = 'AES-128-CTR'
            self._enc_cbc_alg  = 'AES-128-CBC'
            self._enc_ecb_alg  = 'AES-128-ECB'
        elif alg == CfgValue.CryptoAlg.SM:
            self._asym_alg     = 'SM2'
            self._digest_alg   = 'SM3'
            self._enc_ctr_alg  = 'SM4-CTR'
            self._enc_cbc_alg  = 'SM4-CBC'
            self._enc_ecb_alg  = 'SM4-ECB'
        return
    ...
    def enc_ecb(self, data, key, decrypt=False, data_name=None):
        ...
        elif  self._enc_ecb_alg == 'SM4-ECB':
            return sec.enc_sm4_ecb(data, key, decrypt)
