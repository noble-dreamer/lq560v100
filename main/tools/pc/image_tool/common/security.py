...
OPENSSL_TOOL = os.path.abspath('%s/../third_party/openssl' % SCRIPT_DIR)
...
def get_rs_in_signed_file(openssl_sign):
    signature_bytes = openssl_sign
    if signature_bytes[0] != 0x30:
        raise ValueError("Not a valid DER sequence")

    index = 2
    if signature_bytes[index] != 0x02:
        raise ValueError("Expected INTEGER for R")

    r_length = signature_bytes[index + 1]
    if r_length >= 0x22:
        raise ValueError("Invalid length for R")

    if signature_bytes[index + 1] == 0x21 and signature_bytes[index + 2] == 0x00:
        r = signature_bytes[index + 3: index + 3 + (r_length - 1)]
    else:
        r = signature_bytes[index + 2: index + 2 + r_length]

    index += 2 + r_length
    if signature_bytes[index] != 0x02:
        raise ValueError("Expected INTEGER for S")

    s_length = signature_bytes[index + 1]
    if s_length >= 0x22:
        raise ValueError("Invalid length for S")

    if signature_bytes[1] != s_length + r_length + 4:
        raise ValueError("Invalid length for signature")

    if signature_bytes[index + 1] == 0x21 and signature_bytes[index + 2] == 0x00:
        s = signature_bytes[index + 3: index + 3 + (s_length - 1)]
    else:
        s = signature_bytes[index + 2: index + 2 + s_length]
...
def gen_rsa_priv_key(priv_key_file):
    cmd = [OPENSSL_TOOL, 'genrsa', '-f4', '-out', priv_key_file, '4096']
...
def gen_rsa_pub_key(priv_key_file, pub_key_file):
    ...
    cmd = [OPENSSL_TOOL, 'rsa', '-pubout', '-in', priv_key_file,
           '-out', pub_key_file]
    stdout, stderr, return_code = utl.run_cmd(cmd)
...
def enc_aes_128_ecb(data, key, decrypt=False):
    ...
    cmd = [OPENSSL_TOOL, 'enc', '-aes-128-ecb', '-nopad',
...
def enc_sm4_ecb(data, key, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-sm4-ecb', '-nopad',
           mode, '-K', utl.hex2str(key)]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def enc_aes_128_cbc(data, key, iv, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-aes-128-cbc', '-nopad', mode,
...
def enc_aes_256_cbc(data, key, iv, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-aes-256-cbc', '-nopad', mode,
           '-K', utl.hex2str(key), '-iv', utl.hex2str(iv)]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def enc_aes_128_ctr(data, key, iv, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-aes-128-ctr', mode,
           '-K', utl.hex2str(key), '-iv', utl.hex2str(iv)]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def enc_sm4_cbc(data, key, iv, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-sm4-cbc', '-nopad', mode,
           '-K', utl.hex2str(key), '-iv', utl.hex2str(iv)]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def enc_sm4_ctr(data, key, iv, decrypt=False):
    ...
    mode = '-d' if (decrypt) else '-e'
    cmd = [OPENSSL_TOOL, 'enc', '-sm4-ctr', '-nopad', mode,
           '-K', utl.hex2str(key), '-iv', utl.hex2str(iv)]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def dgst_sha256(data):
    cmd = [OPENSSL_TOOL, 'dgst', '-sha256', '-binary', '-r']
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def dgst_sm3(data):
    cmd = [OPENSSL_TOOL, 'dgst', '-SM3', '-binary', '-r']
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
...
def sign_rsa_sha256(data, priv_key_file):
    ...
    cmd = [OPENSSL_TOOL, 'dgst', '-sha256', '-sign', priv_key_file,
...
def sign_sm2_sm3(data, priv_key_file):
    ...
    cmd = [OPENSSL_TOOL, 'pkeyutl', '-sign', '-inkey', priv_key_file,
...
def verify_ecc_sha256(data, sign, pub_key):
    ...
    cmd = [OPENSSL_TOOL, 'dgst', '-sha256', '-verify', pub_key_file,
           '-signature', sign_file]
    stdout, stderr, return_code = utl.run_cmd(cmd, data)
    os.remove(sign_file)
...
def verify_sm2_sm3(data, sign, pub_key):
    ...
    cmd = [OPENSSL_TOOL, 'pkeyutl', '-verify', '-pubin', '-inkey', pub_key_file,
...
def get_publickey_from_pem_file(public_Key_file):
    ...
    cmd = [OPENSSL_TOOL, 'ec', '-in', public_Key_file, '-pubin', '-text']
