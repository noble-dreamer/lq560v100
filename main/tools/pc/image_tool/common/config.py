...
class Scenario:
    def __init__(self, sec_mode=None, crypto_alg=None, start_flow=None,
                 tee_owner=None, otfl_enc=None, system_enc=None, atf_tee_enc=None, cfct_data_enc=None, value=None):
    ...
    def log(self):
        ...
        if self._otfl_enc is not None:
            log.info('  - Encrypt OTFL Code: %s' % d.get(self._otfl_enc))
        if self._system_enc is not None:
            log.info('  - Encrypt Boot Code: %s' % d.get(self._system_enc))
    ...
    def otfl_enc(self):
        return self._otfl_enc

    def system_enc(self):
        return self._system_enc
    ...
    def is_otfl_enc(self):
        return self._otfl_enc != CfgValue.EncFlag.NON_ENC

    def is_system_enc(self):
        return self._system_enc != CfgValue.EncFlag.NON_ENC
    ...
    def is_matched(self, scen):
        ...
        if self._tee_owner is not None and self.tee_owner() != scen.tee_owner():
            return False
        if self._otfl_enc is not None and self.is_otfl_enc() != scen.is_otfl_enc():
            return False
...
class CfgItem:
    def __init__(self, key=None, typ=None, usage='', scens=[Scenario()],
        ...
        if len(sub_items) > 1 and len(scens) > 1:
            log.error("A parent item must be with single scenario")
            exit(1)
        self._key = key
        self._typ = typ
    ...
    def __convert_val(self, value):
        ...
        if self._typ == CfgValueType.UINT32:
            ...
            path = os.path.abspath(value)
            if chk.check_file_is_exist(path) == True:
                ret = utl.read_file(path, data_name=self._key)
            else:
                log.warn("%s No Such File." %(path));
                ret = None
                return ret;
            ...
            if value != "" and (value[0:2] != "/*" and value[len(value) - 2:len(value)] != "*/"):
                ...
                ret = None
                return ret
        elif self._typ == CfgValueType.IN_FILE_LIST:
            dict_tmp={}
            for f in value:
                path = os.path.abspath(f)
                chk.check_file_exist(f)
                dict_tmp[f] = utl.read_file(path, data_name=self._key)
            ret = dict_tmp
...
class Configurator:
    ...
    def load(self, cfg_file):
        ...
        otfl_enc    = cfg_dict.get('OTFL', {}).get('OTFL_Info_area', {}).get('OTFL_code_enc_flag', None)
        system_enc   = cfg_dict.get('SYSTEM', {}).get('system_info_area', {}).get('system_enc_flag', None)
    ...
    def get_value(self, key, default=None):
        ...
        if value == None:
            log.error('get %s value is None.' %(key))
            sys.exit(1)
        return value

    def get_value_optional(self, key, default=None):
        value = self._sub_obj.get(key, default)
        return value
...
def _get_scen_item_from_user(item):
    chk.check_option(
            val=item, notice='Scenario Item', options=[
                ...
                'OTFL_code_enc_flag', 'Boot_Enc_Flag',
    ...
    if owner == CfgValue.Owner.OEM:
        if sec_mode != CfgValue.SecMode.SECURE:
            ...
            crypto_alg = crypto_alg,
                ...
                otfl_enc    = CfgValue.EncFlag.NON_ENC,
                system_enc   = CfgValue.EncFlag.NON_ENC,
        ...
        cfct_data_enc = _get_scen_item_from_user('CFCT_Data_Enc_Flag')
        otfl_enc = _get_scen_item_from_user('OTFL_code_enc_flag')
        system_enc = _get_scen_item_from_user('Boot_Enc_Flag')
