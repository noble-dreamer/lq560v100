...
#ifdef CFG_EMMC_QUICKBOOT_MODE
static void enable_emmc_quick_mode(void)
{
    u32 channel_type = BOOT_SEL_UNKNOW;
    struct regentry reg;
    channel_type = get_data_channel_type();
    /*if boot from sdio, don't enable emmc quickboot*/
    if (channel_type == BOOT_SEL_SDIO) {
      return;
    }
    reg.attr = 0x0000B00D;
    reg.delay = 0;
    reg.reg_addr = 0x11120024;
    reg.value = 0x2;
    reg_write(&reg);
}
#endif
