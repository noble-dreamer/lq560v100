...
#ifdef CONFIG_VENDOR_SEMU
    if(osal_interrupt_mod_report_list(name, -1)) {
        ret = osal_interrupt_report_mod_register(irq, handler, thread_fn, name, dev, flags);
        if (ret != OSAL_SUCCESS) {
            osal_log("osal irq semu register failed! name[%s] ret = %#x.\n", name, ret);
            return OSAL_FAILURE;
        }
        return OSAL_SUCCESS;
    }
...
#ifdef CONFIG_VENDOR_SEMU
    if(osal_interrupt_mod_report_list(NULL, _irq)) {
        if (osal_interrupt_report_mod_unregister(_irq, dev) != OSAL_SUCCESS) {
            osal_log("osal irq semu unregister failed! irq[%u]\n", irq);
        }
        return;
    }
#endif
