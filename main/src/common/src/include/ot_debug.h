...
int OT_LOG(ot_s32 level, ot_mod_id enModId, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
...
typedef struct {
    ot_mod_id  mod_id;
    ot_s32  level;
    ot_char mod_name[OT_MAX_MOD_NAME_LEN];
} ot_log_level_cfg;
...
int ot_log(ot_s32 level, ot_mod_id mod_id, const char *fmt, ...) __attribute__((format(printf, 3, 4)));
