...
接收器布局：右视图已从显示中去掉；左视图（带检测框/距离，板端字体内置 3× 放大）放大到 1024×864，视差图以原生 640×448 显示，右侧信息面板新增「Targets/Classes」列表，逐行列出 `类别: 距离`（来自 perf 帧新增的 `dets` 字段：`[[class_id, distance_mm],...]`）。窗口 1344×1368，过大时加 `--scale 0.75` 整体缩放。
...
- 矫正边缘/无有效视差区域模型视差会**饱和到最大候选 128px（实测 ~137px）**，前端 JET 配色把 `≥118px` 画红、`≥128px` 顶红，因此远景边缘会被误显为红色块。板端已把 `Q5≥4096`（disp≥128px）掩膜为 0（无效），主机端黑色渲染与框中心距离跳过自动生效。
