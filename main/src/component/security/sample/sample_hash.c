/*
  Copyright (c), 2021-2023, Oritek Tech. Co., Ltd.
 */

#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
#include <unistd.h>
#include <sys/ioctl.h>
#include <sys/mman.h>
#include <assert.h>
#include <linux/fb.h>
#include "securec.h"

#include "sample_utils.h"
#include "ot_omi_cipher.h"
#include "ot_omi_km.h"
#include "ot_omi_otp.h"
#include "ot_eis_errno.h"
#include "sample_func.h"

#define QUANTILE_TWO       2
#define QUANTILE_THREE     3
#define DATA_SIZE        512

typedef struct {
    const ot_char *hash_name;
    ot_u8 message[DATA_SIZE];
    ot_u32 message_length;
    crypto_hash_attr hash_attr;
    ot_u32 hash_length;
} hash_data_t;

typedef struct {
    const ot_char *hash_name;
    ot_u8 key[HMAC_KEY_LEN];
    ot_u32 key_length;
    ot_u8 message[DATA_SIZE];
    ot_u32 message_length;
    crypto_hash_attr hash_attr;
    ot_u32 hash_length;
    km_klad_hmac_type klad_hmac_type;
} hmac_data_t;

static hash_data_t g_hash_data[] = {
    {
        .hash_name = "hash-sha256", .message_length = DATA_SIZE, .hash_length = SHA256_HASH_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_SHA256,
            .is_long_term = OT_FALSE
        }
    },
    {
        .hash_name = "hash-sha384", .message_length = DATA_SIZE, .hash_length = SHA384_HASH_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_SHA384,
            .is_long_term = OT_FALSE
        }
    },
    {
        .hash_name = "hash-sha512", .message_length = DATA_SIZE, .hash_length = SHA512_HASH_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_SHA512,
            .is_long_term = OT_FALSE
        }
    }
};

static hmac_data_t g_hmac_data[] = {
    {
        .hash_name = "hmac-sha256", .message_length = DATA_SIZE,
        .hash_length = SHA256_HASH_LEN, .key_length = HMAC_KEY_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_HMAC_SHA256,
            .is_long_term = OT_FALSE
        },
        .klad_hmac_type = KM_KLAD_HMAC_SHA256
    },
    {
        .hash_name = "hmac-sha384", .message_length = DATA_SIZE,
        .hash_length = SHA384_HASH_LEN, .key_length = HMAC_KEY_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_HMAC_SHA384,
            .is_long_term = OT_FALSE
        },
        .klad_hmac_type = KM_KLAD_HMAC_SHA384
    },
    {
        .hash_name = "hmac-sha512", .message_length = DATA_SIZE,
        .hash_length = SHA512_HASH_LEN, .key_length = HMAC_KEY_LEN,
        .hash_attr = {
            .hash_type = CRYPTO_HASH_TYPE_HMAC_SHA512,
            .is_long_term = OT_FALSE
        },
        .klad_hmac_type = KM_KLAD_HMAC_SHA512
    }
};

static ot_s32 hash_set_clear_key(crypto_handle keyslot_crypto_handle, ot_u8 *key, ot_u32 keylen,
    km_klad_hmac_type klad_hmac_type)
{
    ot_s32 ret = OT_SUCCESS;
    crypto_handle klad_handle = 0;
    ot_u32 offset = 0x12;
    ot_u8 tee_enable = 0;
    km_klad_attr klad_attr = {
        .key_cfg = {
            .engine = KM_CRYPTO_ALG_HMAC_SHA2,
            .decrypt_support = OT_TRUE,
            .encrypt_support = OT_TRUE
        }
    };
    km_klad_clear_key clear_key = {
        .hmac_type = klad_hmac_type,
        .key = key,
        .key_size = keylen
    };
    (ot_void)ot_omi_otp_init();
    (ot_void)ot_omi_otp_read_byte(offset, &tee_enable);
    (ot_void)ot_omi_otp_deinit();
    if (tee_enable == 0x42) {
        klad_attr.key_sec_cfg.key_sec = KM_KLAD_SEC_ENABLE;
        klad_attr.key_sec_cfg.master_only_enable = OT_TRUE;
        klad_attr.key_sec_cfg.dest_buf_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_non_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_non_sec_support = OT_FALSE;
    } else {
        klad_attr.key_sec_cfg.key_sec = KM_KLAD_SEC_DISABLE;
        klad_attr.key_sec_cfg.master_only_enable = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_non_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.src_buf_non_sec_support = OT_TRUE;
    }

    /* 1. klad create handle */
    sample_chk_expr_return(ot_omi_klad_create(&klad_handle), OT_SUCCESS);

    /* 2. klad set attr for clear key */
    sample_chk_expr_goto(ot_omi_klad_set_attr(klad_handle, &klad_attr), OT_SUCCESS, __KLAD_DESTORY__);

    /* 3. attach klad handle & kslot handle */
    sample_chk_expr_goto(ot_omi_klad_attach(klad_handle, KM_KLAD_DEST_TYPE_HMAC, keyslot_crypto_handle),
        OT_SUCCESS, __KLAD_DESTORY__);

    /* 4. set clear key */
    sample_chk_expr_goto(ot_omi_klad_set_clear_key(klad_handle, &clear_key), OT_SUCCESS, __KLAD_DETACH__);

__KLAD_DETACH__:
    ot_omi_klad_detach(klad_handle, KM_KLAD_DEST_TYPE_HMAC, keyslot_crypto_handle);
__KLAD_DESTORY__:
    ot_omi_klad_destroy(klad_handle);
    return ret;
}

static ot_s32 hash_set_root_key(crypto_handle keyslot_crypto_handle, ot_u8 *skey, ot_u8 *ckey, ot_u32 slen, ot_u32 clen)
{
    ot_s32 ret = OT_SUCCESS;
    crypto_handle klad_handle = 0;
    ot_u32 offset = 0x12;
    ot_u8 tee_enable = 0;
    km_klad_attr klad_attr = {
        .klad_cfg = {
            .rootkey_type = KM_KLAD_KEY_TYPE_ABRK0
        },
        .key_cfg = {
            .engine = KM_CRYPTO_ALG_HMAC_SHA2,
            .decrypt_support = OT_TRUE,
            .encrypt_support = OT_TRUE
        }
    };
    km_klad_session_key klad_sessino_key = {
        .level = KM_KLAD_LEVEL1,
        .alg = KM_KLAD_ALG_TYPE_AES,
        .key_size = slen,
        .key = skey
    };

    km_klad_content_key klad_content_key = {
        .alg = KM_KLAD_ALG_TYPE_AES,
        .key_size = clen,
        .key = ckey
    };

    (ot_void)ot_omi_otp_init();
    (ot_void)ot_omi_otp_read_byte(offset, &tee_enable);
    (ot_void)ot_omi_otp_deinit();
    if (tee_enable == 0x42) {
        klad_attr.key_sec_cfg.key_sec = KM_KLAD_SEC_ENABLE;
        klad_attr.key_sec_cfg.master_only_enable = OT_TRUE;
        klad_attr.key_sec_cfg.dest_buf_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_non_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_non_sec_support = OT_FALSE;
    } else {
        klad_attr.key_sec_cfg.key_sec = KM_KLAD_SEC_DISABLE;
        klad_attr.key_sec_cfg.master_only_enable = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.dest_buf_non_sec_support = OT_TRUE;
        klad_attr.key_sec_cfg.src_buf_sec_support = OT_FALSE;
        klad_attr.key_sec_cfg.src_buf_non_sec_support = OT_TRUE;
    }

    /* 1. klad create handle */
    sample_chk_expr_return(ot_omi_klad_create(&klad_handle), OT_SUCCESS);

    /* 2. klad set attr for rootkey */
    sample_chk_expr_goto(ot_omi_klad_set_attr(klad_handle, &klad_attr), OT_SUCCESS, __KLAD_DESTORY__);

    /* 3. attach klad handle & kslot handle */
    sample_chk_expr_goto(ot_omi_klad_attach(klad_handle, KM_KLAD_DEST_TYPE_HMAC, keyslot_crypto_handle),
        OT_SUCCESS, __KLAD_DESTORY__);

    /* 4. set session key */
    sample_chk_expr_goto(ot_omi_klad_set_session_key(klad_handle, &klad_sessino_key), OT_SUCCESS, __KLAD_DETACH__);

    /* 5. set content key */
    sample_chk_expr_goto(ot_omi_klad_set_content_key(klad_handle, &klad_content_key), OT_SUCCESS, __KLAD_DETACH__);

__KLAD_DETACH__:
    ot_omi_klad_detach(klad_handle, KM_KLAD_DEST_TYPE_HMAC, keyslot_crypto_handle);
__KLAD_DESTORY__:
    ot_omi_klad_destroy(klad_handle);
    return ret;
}

static ot_s32 run_hash_sample(hash_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 result_len;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, data->message_length), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len), OT_SUCCESS,
        __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hmac_sample(hmac_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 result_len;

    data->hash_attr.is_keyslot = OT_FALSE;
    data->hash_attr.key = data->key;
    data->hash_attr.key_len = data->key_length;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, data->message_length), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len), OT_SUCCESS,
        __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hmac_clear_key_sample(hmac_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle = 0;
    crypto_handle keyslot_crypto_handle = 0;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 result_len;

    data->hash_attr.is_keyslot = OT_TRUE;
    data->hash_attr.is_long_term = OT_TRUE;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. km init */
    sample_chk_expr_goto(ot_omi_km_init(), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. create keyslot handle */
    sample_chk_expr_goto(ot_omi_keyslot_create(&keyslot_crypto_handle, KM_KEYSLOT_TYPE_HMAC), OT_SUCCESS,
        __KM_DEINIT__);

    /* 4. set clear key */
    sample_chk_expr_goto(hash_set_clear_key(keyslot_crypto_handle, data->key, data->key_length, data->klad_hmac_type),
        OT_SUCCESS, __KEYSLOT_DESTROY__);

    /* 5. hash create */
    data->hash_attr.drv_keyslot_crypto_handle = (ot_crypto_handle)keyslot_crypto_handle;
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __KEYSLOT_DESTROY__);

    /* 6. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, data->message_length), OT_SUCCESS,
        __KEYSLOT_DESTROY__);

    /* 7. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len), OT_SUCCESS,
        __KEYSLOT_DESTROY__);

__KEYSLOT_DESTROY__:
    ot_omi_keyslot_destroy(keyslot_crypto_handle);
__KM_DEINIT__:
    ot_omi_km_deinit();
__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hmac_root_key_sample(hmac_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle = 0;
    crypto_handle keyslot_crypto_handle = 0;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 result_len;
    ot_u8 skey[AES128_KEY_LEN];
    ot_u8 ckey[AES256_KEY_LEN];

    sample_chk_expr_return(get_random_data(skey, sizeof(skey)), OT_SUCCESS);
    sample_chk_expr_return(get_random_data(ckey, sizeof(ckey)), OT_SUCCESS);
    data->hash_attr.is_keyslot = OT_TRUE;
    data->hash_attr.is_long_term = OT_TRUE;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. km init */
    sample_chk_expr_goto(ot_omi_km_init(), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. create keyslot handle */
    sample_chk_expr_goto(ot_omi_keyslot_create(&keyslot_crypto_handle, KM_KEYSLOT_TYPE_HMAC), OT_SUCCESS,
        __KM_DEINIT__);

    /* 4. set root key */
    sample_chk_expr_goto(hash_set_root_key(keyslot_crypto_handle, skey, ckey, sizeof(skey), sizeof(ckey)),
        OT_SUCCESS, __KEYSLOT_DESTROY__);

    /* 5. hash create */
    data->hash_attr.drv_keyslot_crypto_handle = (ot_crypto_handle)keyslot_crypto_handle;
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __KEYSLOT_DESTROY__);

    /* 6. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, data->message_length), OT_SUCCESS,
        __KEYSLOT_DESTROY__);

    /* 7. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len), OT_SUCCESS,
        __KEYSLOT_DESTROY__);

__KEYSLOT_DESTROY__:
    ot_omi_keyslot_destroy(keyslot_crypto_handle);
__KM_DEINIT__:
    ot_omi_km_deinit();
__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hash_clone_sample(hash_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    ot_crypto_handle hash_handle_clone;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u8 out_hash_clone[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    crypto_hash_clone_ctx clone_ctx = {0};
    ot_u32 message_length = data->message_length;
    ot_u32 result_len;
    ot_u32 result_len_clone;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle_clone, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, message_length / QUANTILE_TWO), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. clone */
    /* 4.1. get context */
    sample_chk_expr_goto(ot_omi_cipher_hash_get(hash_handle, &clone_ctx), OT_SUCCESS, __HASH_DEINIT__);
    /* 4.2. set context */
    sample_chk_expr_goto(ot_omi_cipher_hash_set(hash_handle_clone, &clone_ctx), OT_SUCCESS, __HASH_DEINIT__);

    /* 5. clone continue update */
    src_buf.virt_addr = data->message + message_length / QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf,
        message_length - message_length / QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + message_length / QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_clone, &src_buf,
        message_length - message_length / QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);

    /* 6. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len),
        OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle_clone, out_hash_clone, out_len, &result_len_clone),
        OT_SUCCESS, __HASH_DEINIT__);

    sample_chk_expr_goto_with_ret(result_len == result_len_clone, OT_TRUE, ret, OT_FAILURE, __HASH_DEINIT__);
    sample_chk_expr_goto_with_ret(memcmp(out_hash, out_hash_clone, result_len),
        0, ret, OT_FAILURE, __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hmac_clone_sample(hmac_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    ot_crypto_handle hash_handle_clone;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u8 out_hash_clone[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    crypto_hash_clone_ctx clone_ctx = {0};
    ot_u32 message_length = data->message_length;
    ot_u32 result_len;
    ot_u32 result_len_clone;

    data->hash_attr.is_keyslot = OT_FALSE;
    data->hash_attr.key = data->key;
    data->hash_attr.key_len = data->key_length;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle_clone, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, message_length / QUANTILE_TWO), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. clone */
    /* 4.1. get context */
    sample_chk_expr_goto(ot_omi_cipher_hash_get(hash_handle, &clone_ctx), OT_SUCCESS, __HASH_DEINIT__);
    /* 4.2. set context */
    sample_chk_expr_goto(ot_omi_cipher_hash_set(hash_handle_clone, &clone_ctx), OT_SUCCESS, __HASH_DEINIT__);

    /* 5. clone continue update */
    src_buf.virt_addr = data->message + message_length / QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf,
        message_length - message_length / QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + message_length / QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_clone, &src_buf,
        message_length - message_length / QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);

    /* 6. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len),
        OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle_clone, out_hash_clone, out_len, &result_len_clone),
        OT_SUCCESS, __HASH_DEINIT__);

    sample_chk_expr_goto_with_ret(result_len == result_len_clone, OT_TRUE, ret, OT_FAILURE, __HASH_DEINIT__);
    sample_chk_expr_goto_with_ret(memcmp(out_hash, out_hash_clone, result_len),
        0, ret, OT_FAILURE, __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hash_multi_update_sample(hash_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    ot_crypto_handle hash_handle_multi;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u8 out_hash_multi[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 message_length = data->message_length;
    ot_u32 result_len;
    ot_u32 result_len_multi;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle_multi, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, message_length), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. hash multi update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf, message_length / QUANTILE_THREE),
        OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + message_length / QUANTILE_THREE;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf, message_length / QUANTILE_THREE),
        OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + (message_length / QUANTILE_THREE) * QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf,
        message_length - (message_length / QUANTILE_THREE) * QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);

    /* 5. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len),
        OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle_multi, out_hash_multi, out_len, &result_len_multi),
        OT_SUCCESS, __HASH_DEINIT__);

    sample_chk_expr_goto_with_ret(result_len == result_len_multi, OT_TRUE, ret, OT_FAILURE, __HASH_DEINIT__);
    sample_chk_expr_goto_with_ret(memcmp(out_hash, out_hash_multi, result_len),
        0, ret, OT_FAILURE, __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 run_hmac_multi_update_sample(hmac_data_t *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_crypto_handle hash_handle;
    ot_crypto_handle hash_handle_multi;
    crypto_buf_attr src_buf = {0};
    ot_u8 out_hash[MAX_HASH_LEN] = {0};
    ot_u8 out_hash_multi[MAX_HASH_LEN] = {0};
    ot_u32 out_len = MAX_HASH_LEN;
    ot_u32 message_length = data->message_length;
    ot_u32 result_len;
    ot_u32 result_len_multi;

    data->hash_attr.is_keyslot = OT_FALSE;
    data->hash_attr.key = data->key;
    data->hash_attr.key_len = data->key_length;

    /* 1. hash init */
    sample_chk_expr_return(ot_omi_cipher_hash_init(), OT_SUCCESS);

    /* 2. hash create */
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_create(&hash_handle_multi, &data->hash_attr), OT_SUCCESS, __HASH_DEINIT__);

    /* 3. hash update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle, &src_buf, message_length), OT_SUCCESS,
        __HASH_DEINIT__);

    /* 4. hash multi update */
    src_buf.virt_addr = data->message;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf, message_length / QUANTILE_THREE),
        OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + message_length / QUANTILE_THREE;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf, message_length / QUANTILE_THREE),
        OT_SUCCESS, __HASH_DEINIT__);
    src_buf.virt_addr = data->message + (message_length / QUANTILE_THREE) * QUANTILE_TWO;
    sample_chk_expr_goto(ot_omi_cipher_hash_update(hash_handle_multi, &src_buf,
        message_length - (message_length / QUANTILE_THREE) * QUANTILE_TWO), OT_SUCCESS, __HASH_DEINIT__);

    /* 5. hash finish */
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle, out_hash, out_len, &result_len),
        OT_SUCCESS, __HASH_DEINIT__);
    sample_chk_expr_goto(ot_omi_cipher_hash_finish(hash_handle_multi, out_hash_multi, out_len, &result_len_multi),
        OT_SUCCESS, __HASH_DEINIT__);

    sample_chk_expr_goto_with_ret(result_len == result_len_multi, OT_TRUE, ret, OT_FAILURE, __HASH_DEINIT__);
    sample_chk_expr_goto_with_ret(memcmp(out_hash, out_hash_multi, result_len),
        0, ret, OT_FAILURE, __HASH_DEINIT__);

__HASH_DEINIT__:
    ot_omi_cipher_hash_deinit();
    return ret;
}

static ot_s32 sample_hash_cal(ot_void)
{
    ot_u32 i;
    ot_s32 ret;
    ot_u32 num = (ot_u32)(sizeof(g_hash_data) / sizeof(g_hash_data[0]));
    sample_log("************ test hash data start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hash_sample(&g_hash_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s failed ************\n", g_hash_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s success ************\n", g_hash_data[i].hash_name);
    }
    sample_log("************ test hash data done ************\n");
    num = (ot_u32)(sizeof(g_hmac_data) / sizeof(g_hmac_data[0]));
    sample_log("************ test hmac data start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hmac_sample(&g_hmac_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s failed ************\n", g_hmac_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s success ************\n", g_hmac_data[i].hash_name);
    }
    sample_log("************ test hmac data done ************\n");
    sample_log("************ test hmac clear key start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hmac_clear_key_sample(&g_hmac_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s failed ************\n", g_hmac_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s success ************\n", g_hmac_data[i].hash_name);
    }
    sample_log("************ test hmac clear key done ************\n");
    sample_log("************ test hmac root key start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hmac_root_key_sample(&g_hmac_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s failed ************\n", g_hmac_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s success ************\n", g_hmac_data[i].hash_name);
    }
    sample_log("************ test hmac root key done ************\n");
    num = (ot_u32)(sizeof(g_hash_data) / sizeof(g_hash_data[0]));
    sample_log("************ test hash clone start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hash_clone_sample(&g_hash_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s clone failed ************\n", g_hash_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s clone success ************\n", g_hash_data[i].hash_name);
    }
    sample_log("************ test hash clone done ************\n");
    num = (ot_u32)(sizeof(g_hmac_data) / sizeof(g_hmac_data[0]));
    sample_log("************ test hmac clone start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hmac_clone_sample(&g_hmac_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s clone failed ************\n", g_hmac_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s clone success ************\n", g_hmac_data[i].hash_name);
    }
    sample_log("************ test hmac clone done ************\n");
    num = (ot_u32)(sizeof(g_hash_data) / sizeof(g_hash_data[0]));
    sample_log("************ test hash multi update start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hash_multi_update_sample(&g_hash_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s multi update failed ************\n", g_hash_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s multi update success ************\n", g_hash_data[i].hash_name);
    }
    sample_log("************ test hash multi update done ************\n");
    num = (ot_u32)(sizeof(g_hmac_data) / sizeof(g_hmac_data[0]));
    sample_log("************ test hmac multi update start ************\n");
    for (i = 0; i < num; i++) {
        ret = run_hmac_multi_update_sample(&g_hmac_data[i]);
        if (ret != OT_SUCCESS) {
            sample_err("************ test %s multi update failed ************\n", g_hmac_data[i].hash_name);
            return ret;
        }
        sample_log("************ test %s multi update success ************\n", g_hmac_data[i].hash_name);
    }
    sample_log("************ test hmac multi update done ************\n");
    return OT_SUCCESS;
}

static ot_s32 data_init(ot_void)
{
    ot_u32 i;
    ot_u32 num = 0;
    /* 1. init g_hash_data */
    num = (ot_u32)(sizeof(g_hash_data) / sizeof(g_hash_data[0]));
    for (i = 0; i < num; i++) {
        sample_chk_expr_return(get_random_data(g_hash_data[i].message, sizeof(g_hash_data[i].message)), OT_SUCCESS);
    }
    /* 2. init g_hmac_data */
    num = (ot_u32)(sizeof(g_hmac_data) / sizeof(g_hmac_data[0]));
    for (i = 0; i < num; i++) {
        sample_chk_expr_return(get_random_data(g_hmac_data[i].message, sizeof(g_hmac_data[i].message)), OT_SUCCESS);
        sample_chk_expr_return(get_random_data(g_hmac_data[i].key, sizeof(g_hmac_data[i].key)), OT_SUCCESS);
    }
    return OT_SUCCESS;
}

static ot_void data_deinit(ot_void)
{
    ot_u32 i;
    ot_u32 num = 0;
    /* 1. clear the key g_hmac_data */
    num = (ot_u32)(sizeof(g_hmac_data) / sizeof(g_hmac_data[0]));
    for (i = 0; i < num; i++) {
        memset_s(g_hmac_data[i].key, sizeof(g_hmac_data[i].key), 0, sizeof(g_hmac_data[i].key));
    }
}

ot_s32 sample_hash(ot_void)
{
    ot_s32 ret;
    sample_chk_expr_return(data_init(), OT_SUCCESS);
    sample_log("************ test hash ************\n");
    ret = sample_hash_cal();
    if (ret != OT_SUCCESS) {
        return ret;
    }
    sample_log("************ test hash succeed ************\n");
    data_deinit();
    return OT_SUCCESS;
}
