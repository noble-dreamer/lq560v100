...
typedef struct {
    ...
    drv_pke_ecc_curve_type curve_type;
    ot_u32 key_len;
    ot_u32 plain_data_len;
    ot_u32 cipher_data_len;
} ecc_crypto_data;
...
/* SM2 generate key */
static ot_s32 run_sm2_generate_key_sample(ecc_data *data)
{
...
/* SM2 ecdh */
static ot_s32 run_sm2_ecdh_sample(ecc_data *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_u32 length;
...
/* SM2 sign */
static ot_s32 run_sm2_sign_sample(ecc_data *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_bool is_on_curve = OT_FALSE;
    ot_u8 d[MAX_ECC_LENGTH] = {0};
    ...
    sample_chk_expr_return(get_random_data(input_msg, sizeof(input_msg)), OT_SUCCESS);
    sample_chk_expr_return(get_random_data(sm2_id_buf, sizeof(sm2_id_buf)), OT_SUCCESS);
...
/* SM2 crypto */
static ot_s32 run_sm2_crypto_sample(ecc_crypto_data *data)
{
    ot_s32 ret = OT_SUCCESS;
    ot_bool is_on_curve = OT_FALSE;
    ot_u8 d[MAX_ECC_LENGTH] = {0};
    ...
    sample_chk_expr_return(plain_data != OT_NULL, OT_TRUE);
    cipher_data = malloc(data->cipher_data_len);
    sample_chk_expr_goto(cipher_data != OT_NULL, OT_TRUE, __EXIT_FREE_PLAIN__);
    check_data = malloc(data->plain_data_len);
    sample_chk_expr_goto(plain_data != OT_NULL, OT_TRUE, __EXIT_FREE_CIPHER__);

    sample_chk_expr_goto(get_random_data(plain_data, data->plain_data_len), OT_SUCCESS, __EXIT_FREE_CHECK__);
    ...
    /* 1. Pke Init */
    sample_chk_expr_goto(ot_omi_cipher_pke_init(), OT_SUCCESS, __EXIT_FREE_CHECK__);
    ...
    /* 3. Check Dot */
    sample_chk_expr_goto(ot_omi_cipher_pke_check_dot_on_curve(data->curve_type, &pub_key, &is_on_curve),
        OT_SUCCESS, __PKE_DEINIT__);

    /* 4. SM2 Public Encrypt */
    sample_chk_expr_goto(ot_omi_cipher_pke_sm2_public_encrypt(&pub_key, &plain_text, &cipher_text),
        OT_SUCCESS, __PKE_DEINIT__);

    /* 5. SM2 Private Decrypt */
    sample_chk_expr_goto(ot_omi_cipher_pke_sm2_private_decrypt(&priv_key, &cipher_text, &check_text),
        OT_SUCCESS, __PKE_DEINIT__);
...
static ot_s32 sample_sm2_cal(ot_void)
{
    ot_u32 i;
    ot_s32 ret;
    ot_u32 num = (ot_u32)(sizeof(g_sm2_data) / sizeof(g_sm2_data[0]));
    for (i = 0; i < num; i++) {
    ...
    num = (ot_u32)(sizeof(g_sm2_crypto_data) / sizeof(g_sm2_crypto_data[0]));
    for (i = 0; i < num; i++) {
