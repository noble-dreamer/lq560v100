...
typedef ot_bool (*drv_kdf_wait_condition_func)(const ot_void *param);

typedef ot_bool (*drv_klad_wait_condition_func)(const ot_void *param);

typedef ot_s32 (*osal_kdf_wait_timeout_uninterruptible)(const ot_void *wait, drv_kdf_wait_condition_func func,
    const ot_void *param, const ot_u32 timeout_ms);

typedef ot_s32 (*osal_klad_wait_timeout_uninterruptible)(const ot_void *wait, drv_klad_wait_condition_func func,
    const ot_void *param, const ot_u32 timeout_ms);
...
typedef struct {
    ot_u32 rootkey_type;  /* Keyladder rootkey type, KM_KLAD_KEY_TYPE_xxx. */
} km_klad_config;
...
typedef struct {
    ...
    ot_u32 rkp_sw_cfg; /* The software configuration for NPU */
} km_klad_attr;
 ...
 */
typedef struct {
    ot_u8 *key;     /* Clear key content. */
    ot_u32 key_length;  /* Length of the clear key, in bytes.
    ...
    ot_bool key_parity; /* Indicates the parity attribute of a key.
...
typedef struct {
    ...
    ot_bool decrypt_support;    /* The working key can be used for decrypting. */
    ot_bool encrypt_support;    /* The working key can be used for encrypting. */
 ...
 */
typedef struct {
    ot_bool key_sec;    /* Secure key can only be used by TEE CPU and AIDSP locked cipher and hash channel. */
    ot_bool master_only_enable; /* Only the cipher or hash channel which is locked by same CPU as keyladder
                                        can use this key, valid only for TEE CPU and AIDSP. */
    ot_bool dest_buf_sec_support;   /* The destination buffer of target engine can be secure. */
    ot_bool dest_buf_non_sec_support; /* The destination buffer of target engine can be secure. */
    ot_bool src_buf_sec_support;      /* The destination buffer of target engine can be secure. */
    ot_bool src_buf_non_sec_support;  /* The destination buffer of target engine can be secure. */
...
typedef struct {
    ...
    ot_u8 *salt;    /* Salt content. Used as user input materials for key derivation.
                            The final working key varies according to the salt value. */
    ot_u32 salt_length; /* Salt length, in bytes. It can only be 28. */
    ot_bool oneway; /* Salt length, in bytes. It can only be 28. */
