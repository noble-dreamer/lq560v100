...
static void vo_bt_pin_mux(int vo_bt_mode)
{
    ...
    sys_writel(iocfg_base2 + 0x0010,    0x100); /* gpio10_3拉高 */
...
static void i2c1_pin_mux(void)
{
    ...
    void *iocfg_base = sys_config_get_reg_iocfg();
    void *iocfg4_base = sys_config_get_reg_iocfg4();

    if (board_type == BOARD_SCK) {
        sys_writel(iocfg4_base + 0x0010, 0x1302); /* I2C1_SDA */
        sys_writel(iocfg4_base + 0x0018, 0x1102); /* I2C1_SCL */
    } else {
...
#if SPI2_7LINE_EN
/* SPI2 7ine use uart3 for gpio */
static void spi2_7line_pin_mux(void)
{
    void *iocfg_base = sys_config_get_reg_iocfg();
    void *gpio_base = sys_config_get_reg_gpio();

    /* GPIO8_5*/
    sys_writel(iocfg_base + 0x00e0, 0x0100);
    /* GPIO8_5 output high */
    sys_writel(gpio_base + 0x8400, sys_read(gpio_base + 0x8400) | 0x20);
    sys_writel(gpio_base + 0x8080, sys_read(gpio_base + 0x8080) | 0x20);

    /* GPIO8_6 */
    sys_writel(iocfg_base + 0x00e4, 0x0100);
    /* GPIO8_6 input*/
    sys_writel(gpio_base + 0x8400, sys_read(gpio_base + 0x8400) & 0xBF);
}
#endif
...
static void dmeb_video_pin_mux(void)
{
    void *iocfg3_base = sys_config_get_reg_iocfg3();
    sys_writel(iocfg3_base + 0x0018, 0x1100); // disable USB_OVRCUR and pullup

    spi_pin_mux(PIN_MUX_SPI_3_GROUP2);
}
...
static void i2s0_pin_mux(int board_type)
{
    ...
    if (board_type == BOARD_SCK) {
        ...
        sys_writel(iocfg_base + 0x00dc, 0x1100); /* RESET-N-AIC31 */
    } else if (board_type == BOARD_DMEBPRO) {
        sys_writel(iocfg_base + 0x0000, 0x1724); /* I2S0_SD_RX */
        sys_writel(iocfg_base + 0x0004, 0x1224); /* I2S0_MCLK */
        sys_writel(iocfg_base + 0x0008, 0x1614); /* I2S0_WS */
        sys_writel(iocfg_base + 0x000C, 0x1224); /* I2S0_SD_TX */
        sys_writel(iocfg_base + 0x0020, 0x1603); /* I2S0_BCLK */
    } else if (board_type == BOARD_DMEB) {
        sys_writel(iocfg_base + 0x0058, 0x1202); /* I2S0_SD_RX */
        sys_writel(iocfg_base + 0x005C, 0x1202); /* I2S0_MCLK */
        sys_writel(iocfg_base + 0x0060, 0x1302); /* I2S0_WS */
        sys_writel(iocfg_base + 0x0064, 0x1612); /* I2S0_SD_TX */
        sys_writel(iocfg_base + 0x0068, 0x1302); /* I2S0_BCLK */
        sys_writel(iocfg_base + 0x00dc, 0x1100); /* RESET-N-AIC31 */
        sys_writel(gpio_base + 0x8400, 0x28); /* RESET-N-AIC31 */
        sys_writel(gpio_base + 0x8080, 0x20); /* RESET-N-AIC31 */
    }
}
#endif
...
void pin_mux(void)
{
    ...
    if (vi_intf_type != VI_THERMO_MODE_T3) { /* mux with thermo_t3 */
        if (board_type != BOARD_SCK) {
            amp_unmute_pin_mux(); // conflict sck board i2c1 scl
        }
    }

#if SPI2_EN
    if (board_type != BOARD_SCK) {
        spi_pin_mux(PIN_MUX_SPI_2); // conflict sck board i2c1 & i2c2
    }
#endif
#if DIS_SPI3_EN
    spi_pin_mux(PIN_MUX_SPI_3_GROUP2);
