...
typedef struct {
    ot_bool        is_solid;                   /* RW; Solid or hollow cover. */
    ot_u32         thick;                      /* RW; Thick of the hollow quadrangle. */
    ot_point       point[OT_QUAD_POINT_NUM];   /* RW; Four points of the quadrangle. */
} ot_quad_cover;

typedef struct {
    ot_bool        is_solid;  /* RW; Solid or hollow cover. */
    ot_u32         thick;     /* RW; Thick of the hollow rectangle. */
...
typedef struct {
    ...
    ot_u32             color;  /* RW; Range: [0,0xFFFFFF]; Color of cover. */
} ot_cover;
...
typedef struct {
    ot_s32  src_frame_rate;        /* RW; source frame rate */
    ot_s32  dst_frame_rate;        /* RW; dest frame rate */
} ot_frame_rate_ctrl;
...
typedef struct {
    ot_s16              offset_top;           /* top offset of show area */
    ot_s16              offset_left;          /* left offset of show area */
    ot_s16              offset_bottom;        /* bottom offset of show area */
    ot_s16              offset_right;         /* right offset of show area */

    ot_u32              max_luma;             /* for HDR */
    ot_u32              min_luma;             /* for HDR */
    ot_u64              private_data[OT_MAX_PRIVATE_DATA_NUM];
} ot_video_supplement_misc;

typedef struct {
    ot_phys_addr_t   misc_info_phys_addr;        /* default allocated buffer */
    ot_phys_addr_t   jpeg_dcf_phys_addr;
    ot_phys_addr_t   isp_info_phys_addr;
    ot_phys_addr_t   low_delay_phys_addr;
    ot_phys_addr_t   bnr_mot_phys_addr;
    ot_phys_addr_t   motion_data_phys_addr;
    ot_phys_addr_t   frame_dng_phys_addr;
    ot_phys_addr_t   aiisp_phys_addr;

    ot_void* ATTRIBUTE misc_info_virt_addr;       /* misc info */
    ot_void* ATTRIBUTE jpeg_dcf_virt_addr;        /* jpeg_dcf, used in JPEG DCF */
    ot_void* ATTRIBUTE isp_info_virt_addr;        /* isp_frame_info, used in ISP debug, when get raw and send raw */
    ot_void* ATTRIBUTE low_delay_virt_addr;       /* used in low delay */
    ot_void* ATTRIBUTE bnr_mot_virt_addr;         /* used for 3dnr from bnr mot */
    ot_void* ATTRIBUTE motion_data_virt_addr;     /* gme motion data, filter motion data, gyro data. */
    ot_void* ATTRIBUTE frame_dng_virt_addr;
    ot_void* ATTRIBUTE aiisp_virt_addr;           /* aiisp info */
} ot_video_supplement;
...
typedef struct {
    ...
    ot_u32              header_stride[OT_MAX_COLOR_COMPONENT];
    ot_u32              stride[OT_MAX_COLOR_COMPONENT];

    ot_phys_addr_t      header_phys_addr[OT_MAX_COLOR_COMPONENT];
    ot_phys_addr_t      phys_addr[OT_MAX_COLOR_COMPONENT];
    ot_void* ATTRIBUTE  header_virt_addr[OT_MAX_COLOR_COMPONENT];
    ot_void* ATTRIBUTE  virt_addr[OT_MAX_COLOR_COMPONENT];

    ot_u32              time_ref;
    ot_u64              pts;

    ot_u64              user_data[OT_MAX_USER_DATA_NUM];
    ot_u32              frame_flag;     /* frame_flag, can be OR operation. */
...
typedef struct {
    ...
    ot_u32 width;               /* bitmap's width */
    ot_u32 height;              /* bitmap's height */
    ot_void* ATTRIBUTE data;      /* address of bitmap's data */
} ot_bmp;
...
typedef struct {
    ot_bool             enable;         /* RW; range: [0, 1]; Rotate enable. */
...
typedef struct {
    ot_u8 image_description[OT_DCF_DESCRIPTION_LENGTH];
    /* shows manufacturer of digital cameras */
    ot_u8 make[OT_DCF_DESCRIPTION_LENGTH];
    /* shows model number of digital cameras */
    ot_u8 model[OT_DCF_DESCRIPTION_LENGTH];
    /* shows firmware (internal software of digital cameras) version number */
    ot_u8 software[OT_DCF_DESCRIPTION_LENGTH];
    /*
...
typedef struct {
    ...
    ot_u8           capture_time[OT_DCF_CAPTURE_TIME_LENGTH];
...
typedef struct {
    ot_u32      iso;                    /* ISP internal ISO : again*dgain*is_pgain */
    ot_u32      exposure_time[OT_ISP_WDR_MAX_FRAME_NUM]; /* exposure time (reciprocal of shutter speed), unit is us */
    ot_u32      isp_dgain[OT_ISP_WDR_MAX_FRAME_NUM];
    ot_u32      again[OT_ISP_WDR_MAX_FRAME_NUM];
    ot_u32      dgain[OT_ISP_WDR_MAX_FRAME_NUM];
    ot_isp_fswdr_mode fs_wdr_mode;

    ot_s32      fe_id[OT_ISP_WDR_MAX_FRAME_NUM];
    ot_u32      ratio[OT_ISP_WDR_MAX_FRAME_NUM - 1];
    ot_u32      isp_nr_strength;
    ot_u32      f_number;               /* the actual F-number (F-stop) of lens when the image was taken */
    ot_u32      sensor_id;              /* which sensor is used */
    ot_u32      sensor_mode;
    ot_u32      hmax_times;             /* sensor hmax_times,unit is ns */
    ot_u32      vmax;                   /* sensor vmax,unit is line */
    ot_u32      vc_num;                 /* when dump wdr frame, which is long or short  exposure frame. */
    ot_u32      wb_gain[OT_ISP_WB_GAIN_NUM]; /* white balance gain info */
    ot_u16      ccm[OT_ISP_CAP_CCM_NUM];     /* RW; Range: [0x0, 0xFFFF]; Format:8.8; Manual CCM matrix. */
    ot_u32      exposure_distance[OT_ISP_WDR_MAX_FRAME_NUM - 1]; /* wdr sensor exposure distance line number. */
    ot_s32      sync_id;
    ot_u32      blc_sync_id;
} ot_isp_frame_info;
...
typedef struct {
    ...
    ot_u16 ies0;     /* RW; Range: [0, 255]; The gains of edge and texture enhancement. */
    ot_u16 ies1;     /* RW; Range: [0, 255]; The gains of edge and texture enhancement. */
    ot_u16 ies2;     /* RW; Range: [0, 255]; The gains of edge and texture enhancement. */
    ot_u16 ies3;     /* RW; Range: [0, 255]; The gains of edge and texture enhancement. */
    ot_u16 aux_ies;  /* RW; Range: [0, 255]; The gains of edge enhancement for NO.2 filter. */
    struct {
        ot_u16 iedz : 10;    /* RW; Range: [0, 999]; The threshold to control the generated artifacts. */
        ot_u16 _rb_ : 6;
    };
    ...
    ot_u8 aux_o_sht;   /* RW; Range: [0, 255]; The gains to control the overshoot. */
    ot_u8 aux_u_sht;   /* RW; Range: [0, 255]; The gains to control the undershoot. */
} ot_nr_v1_adv_iey;

typedef struct {
    struct {
        ot_u16 spn6    : 4;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.6 filter. */
        ot_u16 sbn6    : 4;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.6 filter. */
        ot_u16 pbr6    : 5;  /* RW; Range: [0,  16]; The mix ratio between spn and sbr. */
        ot_u16 j_mode6 : 2;  /* RW; Range: [0,   2]; The selection modes for the blending of spatial filters. */
        ot_u16 _rb_   : 1;  /* RW; Reserved */
    };
    ...
    ot_u8 sfs2;        /* RW; Range: [0, 255]; Currently no use, reserved. */
    ot_u16 bld1;       /* RW; Range: [0, 999];  The NR strength parameters for NO.1 filter. */
    struct {
        ot_u16 spn3  : 3;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.3 filter. */
        ot_u16 sbn3  : 3;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.3 filter. */
        ot_u16 pbr3  : 5;  /* RW; Range: [0,  16]; The mix ratio between spn and sbr. */
        ot_u16 _rb0_ : 5;
    };
    struct {
        ot_u16 spn4  : 3;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.4 filter. */
        ot_u16 sbn4  : 3;  /* RW; Range: [0,   5]; The selection of filters to be mixed for NO.4 filter. */
        ot_u16 pbr4  : 5;  /* RW; Range: [0,  16]; The mix ratio between spn and sbr. */
        ot_u16 _rb1_ : 5;
    };

    ot_u8 sfs5;           /* RW; Range: [0, 255]; The NR strength parameters for NO.5 filters. */
    ot_u8 sft5;           /* RW; Range: [0, 255]; The NR strength parameters for NO.5 filters. */
    ot_u8 sbr5;           /* RW; Range: [0, 255]; The NR strength parameters for NO.5 filters. */
    ot_u8 reserved2;

    ot_u16  sth1;        /* RW; Range: [0, 511]; The thresholds for protection of edges from blurring. */
    ot_u16  sth2;        /* RW; Range: [0, 511]; The thresholds for protection of edges from blurring. */
    ot_u16  sth3;        /* RW; Range: [0, 511]; The thresholds for protection of edges from blurring. */
    struct {
        ot_u16 sfn0 : 4; /* RW; Range: [0, 6]; Filter selection for different foreground image areas based on sth1. */
        ot_u16 sfn1 : 4; /* RW; Range: [0, 6]; Filter selection for different foreground image areas based on sth1. */
        ot_u16 sfn2 : 4; /* RW; Range: [0, 6]; Filter selection for different foreground image areas based on sth2. */
        ot_u16 sfn3 : 4; /* RW; Range: [0, 6]; Filter selection for different foreground image areas based on sth3. */
    };
    ot_u8 bri_str[OT_NR_S_IDX_LEN];   /* RW; Range: [0, 255];  Spatial NR strength based on brightness. */
    ot_u8 bri_idx;                    /* RW; Range: [0, 6]; The filter selection based on brightness */
    ot_u8 k_mode;               /* RW; Range: [0, 1]; mode selection of k filter */
...
typedef struct {
    struct {
        ot_u16 tfs0   : 4;   /* RW; Range: [0,  15]; The NR strength for temporal filtering. */
    ...
    struct {
        ...
        ot_u16 math_mode : 1;   /* RW; Range: [0,   1]; The motion detection mode. */
        ot_u16 _rb1_     : 1;
    };
...
typedef struct {
    ...
    ot_u16 dci_w;          /* RW; Range: [0, 999]; The temporal gain for dci gain */
} ot_nr_v1_mdy;
...
typedef struct {
    ...
    ot_u8 tpc;              /* RW; Range: [0, 1];  Chroma denoise mode. */
...
typedef struct {
    ot_u32                   mdz;                 /* RW; Range: [0, 63]; */
    ot_u32                   hard_tf_strength;    /* RW; Range: [0, 96]; Relation strength of hard NR channel. */
    /* Only for weak nr chn */
    ot_u32                   edz;                 /* RW; Range: [0, 63]; */
    ot_u32                   weak_tf_strength;    /* RW; Range: [0, 63]; Relation strength of weak NR channel. */
} ot_nr_adv_param;
...
typedef struct {
    ot_u32 numerator;   /* represents the numerator of a fraction. */
    ot_u32 denominator; /* the denominator. */
} ot_dng_rational;

/* defines the structure of DNG image dynamic information */
typedef struct {
    /* RO; range: [0x0, 0xFFFF]; black level. */
    ot_u32 black_level[OT_ISP_BAYER_CHN];
    /*
    ...
    ot_double ad_noise_profile[OT_DNG_NP_SIZE];
...
typedef struct {
    ...
    ot_u32 isp_dgain;
    ot_u32 exposure_time;
    ot_u32 white_balance_gain[OT_ISP_WB_GAIN_NUM];
    ot_u32 color_temperature;
    ot_u16 cap_ccm[OT_ISP_CAP_CCM_NUM];
} ot_isp_config_info;
...
typedef struct {
    ...
    ot_bool enable;
    ot_u32  spread_coef; /* RW; range: [0, 18];strength coefficient of spread correction */
} ot_spread_attr;

/* dest_point */
typedef struct {
    ot_s64 motion_data[OT_DIS_MOTIONDATA_NUM];
} ot_dis_motion_data;
...
typedef struct {
    ...
    ot_s32 private_data[OT_DIS_PRIVATE_NUM];
} ot_dis_motion_data_info;

typedef struct {
    ot_bool    aspect;      /* RW;range: [0, 1];whether aspect ration  is keep */
    ot_s32     x_ratio;     /* RW; range: [0, 100]; field angle ration of  horizontal,valid when aspect=0. */
    ot_s32     y_ratio;     /* RW; range: [0, 100]; field angle ration of  vertical,valid when aspect=0. */
    ot_s32     xy_ratio;    /* RW; range: [0, 100]; field angle ration of  all,valid when aspect=1. */
     /* RW; range: [-511, 511]; horizontal offset of the image distortion center relative to image center. */
    ot_s32 center_x_offset;
     /* RW; range: [-511, 511]; vertical offset of the image distortion center relative to image center. */
    ot_s32 center_y_offset;
...
typedef struct {
    ...
    ot_s32 dst_calibration_ratio[OT_DST_LENS_COEF_NUM];
...
typedef struct {
    ot_bool enable;                 /* RW; Range [0,1]; Whether LDC is enable */
...
/* gdc PMF attr */
typedef struct {
    ot_bool enable;                          /* RW; Range [0,1]; Whether or not pmf is enable */
    ot_size dst_size;
    ot_s64  pmf_coef[OT_GDC_COMMON_COEF_NUM]; /* RW; PMF coefficient of gdc */
} ot_pmf_attr;


/* gdc FOV attr */
typedef struct {
    ot_bool enable;                          /* RW; Range [0,1]; Whether or not fov is enable */
    ot_s64 fov_coef[OT_GDC_COMMON_COEF_NUM]; /* RW; matrix for match the two */
} ot_fov_attr;
...
typedef struct {
    ...
    ot_phys_addr_t      lut_phys_addr;  /* phys addr of lut */
    ot_u32              lut_len;        /* length of lut */
} ot_lut;
...
typedef struct {
    ot_bool     pre_lut_en;     /* RW; Range [0,1]; Whether or not the pre lut is enable */
    ot_lut      pre_lut;        /* RW; pre lut */
    ot_bool     post_lut_en;    /* RW; Range [0,1]; Whether or not the post lut is enable */
    ot_lut      post_lut;       /* RW; post lut */
    ot_lut_mode lut_mode;       /* RW; lut mode */
    ot_u32      ratio;          /* RW; range: [0, 32768]; fusion ratio of two lut */
} ot_lut_attr;

typedef struct {
    ot_u32  blend_position; /* RW; blend position, the beginning of the blend range */
    ot_u32  blend_range; /* RW; blend range, within blend area */
} ot_blend_param;

typedef struct {
    ot_u32         buf_num; /* RW; Range [0, 5]; buffer number of blend area */
    ot_blend_param blend_param[OT_GDC_MAX_STITCH_NUM - 1]; /* RW; param of stitch blend */
} ot_stitch_blend_param;

typedef struct {
    ot_s32 isp_pipe[OT_GDC_MAX_STITCH_NUM]; /* W; isp pipe of bind stitch in pipe */
    ot_u32 milli_sec; /* W; Range [100, 10000]; sync time interval, millisecond */
    ot_u8 smooth_strength; /* W; Range [1, 16]; vertical filter window size */
    ot_u8 shadding_adjust_threshold; /* W; Range [0, 255]; shading gain range */
...
typedef struct {
    ot_bool pmf_enable;      /* RW; Range [0, 1]; Whether or not pmf is enable */
    ot_s64  pmf_coef[OT_GDC_COMMON_COEF_NUM]; /* RW; PMF coefficient of gdc */
    ot_s32  center_x_offset; /* RW; offset of x; Range [-511, 511] */
    ot_s32  center_y_offset; /* RW; offset of y; Range [-511, 511] */
    ot_u32  ratio;           /* RW: ratio of cylindrical projection; Range [0, 500]  */
...
typedef struct {
    ot_bool         enable; /* RW; Range [0, 1]; Whether or not stitch is enable */
    ot_u32          pipe_num; /* RW; Range [2, 4]; stitch pipe number */
    ot_bool         sync_pipe_en; /* RW; Range [0, 1]; sync pipe with pts or not */
...
typedef struct {
    ot_bool enable; /* RW; Range: [0, 1]; Zoom enable. */
...
typedef struct {
    ot_bool           share_buf_en;
    ot_u32            frame_buf_ratio;
