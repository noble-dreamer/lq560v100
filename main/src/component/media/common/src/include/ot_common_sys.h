...
typedef struct {
    ot_u32 id[OT_UNIQUE_ID_NUM];
} ot_unique_id;

typedef struct {
    ot_char gps_latitude_ref; /* GPS latitude_ref indicates whether the latitude is north or south latitude,
                               * 'N'/'S', default 'N' */
    ot_u32 gps_latitude[OT_GPS_EXPRESS_NUM][OT_GPS_GRP_NUM];
                               /* GPS latitude is expressed as degrees, minutes and seconds, a typical format
                                   * like "dd/1, mm/1, ss/1", default 0/0, 0/0, 0/0 */
    ot_char gps_longitude_ref; /* GPS longitude_ref indicates whether the longitude is east or west longitude,
                                * 'E'/'W', default 'E' */
    ot_u32 gps_longitude[OT_GPS_EXPRESS_NUM][OT_GPS_GRP_NUM];
                                /* GPS longitude is expressed as degrees, minutes and seconds, a typical format
                                    * like "dd/1, mm/1, ss/1", default 0/0, 0/0, 0/0 */
    ot_u8 gps_altitude_ref; /* GPS altitude_ref indicates the reference altitude used, 0 - above sea level,
                             * 1 - below sea level default 0 */
    ot_u32 gps_altitude[OT_GPS_GRP_NUM];
                             /* GPS altitude_ref indicates the altitude based on the reference gps_altitude_ref,
                                * the reference unit is meters, default 0/0 */
} ot_gps_info;

typedef enum {
    OT_VI_OFFLINE_VPROC_OFFLINE = 0,
    OT_VI_OFFLINE_VPROC_ONLINE,
    OT_VI_ONLINE_VPROC_OFFLINE,
    OT_VI_ONLINE_VPROC_ONLINE,
    OT_VI_PARALLEL_VPROC_OFFLINE,
    OT_VI_PARALLEL_VPROC_PARALLEL,
    OT_VI_VPROC_MODE_BUTT
} ot_vi_vproc_mode_type;

typedef struct {
    ot_vi_vproc_mode_type mode[OT_VI_MAX_PIPE_NUM];
} ot_vi_vproc_mode;
...
typedef struct {
    ot_u32 comp_ratio_10_bit; /* compression ratio of 10 bit */
    ot_u32 comp_ratio_12_bit; /* compression ratio of 12 bit */
    ot_u32 comp_ratio_14_bit; /* compression ratio of 14 bit */
