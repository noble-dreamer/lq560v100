...
ot_s32 sample_dpu_get_lut_from_file(const ot_char *file_name, ot_avp_dpu_rect_mem_info *mem_info)
{
    ...
    ret = ot_smr_flush_cache(mem_info->phys_addr, virt_addr, size);
    sample_avp_dpu_check_exps_goto(ret != EOK, fail_0, "Err, flush smr failed!\n");
...
static ot_void sample_dpu_set_match_grp_depth_attr(sample_dpu_cfg *dpu_cfg)
{
    ...
    dpu_cfg->dpu_match_grp_attr.depth_param.v1.fb = (ot_u32)(dpu_cfg->fxb * (1 << SAMPLE_DPU_MATCH_FB_NORM_BITS) + 0.5);
    dpu_cfg->dpu_match_grp_attr.depth_param.v1.z0 = SAMPLE_DPU_MATCH_Z0;
    dpu_cfg->dpu_match_grp_attr.depth_param.v1.min_depth_thr = 50;
    dpu_cfg->dpu_match_grp_attr.depth_param.v1.max_depth_thr = 16383;
...
static ot_s32 sample_dpu_load_fxb(const char* filename, double* value) 
{
    FILE* file = fopen(filename, "rb");
    if (file == NULL) {
        printf("Error: Cannot open bxf file %s for reading !\n", filename);
        return OT_FAILURE;
    }
    
    size_t read_count = fread(value, sizeof(double), 1, file);
    fclose(file);
    
    if (read_count != 1) {
        printf("Error: Failed to read from bxf file ! \n");
        return OT_FAILURE;
    }
...
ot_s32 sample_dpu_rect_init(sample_dpu_cfg *dpu_cfg, const ot_eis_img_size *src_size, const ot_eis_img_size *dst_size, const ot_char *lut_file[], ot_u32 file_num)
{
    ...
    // load fxb bin
    printf("bin_path = %s\n", lut_file[2]);
    sample_avp_dpu_check_exps_return(((strlen(lut_file[2]) > AVP_DPU_PATH_MAX) || (realpath(lut_file[2], path) == OT_NULL)), OT_FAILURE, "Error, file_name is invalid!\n");
    printf("bin_path read OK %s\n");
    ret = sample_dpu_load_fxb(lut_file[2], &dpu_cfg->fxb);
    printf("bin_path load OK, bxf = %f\n", dpu_cfg->fxb);
    sample_avp_dpu_check_exps_goto(ret != OT_SUCCESS, fail_0, "load fxb failed!\n");
