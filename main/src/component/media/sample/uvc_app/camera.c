...
{
    camera_register_uvc_mpi_ops();

    for (ot_u32 i = 0; i < g_uvc_dev_cnt; i++) {
...
{
    for (ot_u32 i = 0; i < g_uvc_dev_cnt; i++) {
        pthread_create(&g_st_uvc_pid[i], 0, uvc_thread, (void *)i);
    }
