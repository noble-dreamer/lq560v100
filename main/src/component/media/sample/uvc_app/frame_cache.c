...
int uvc_frame_nodes_alloc(uvc_cache_t* uvc_cache, unsigned int frame_size)
{
    ...
    */
    return create_cache_node_list(uvc_cache->free_queue, frame_size, DEFALT_PAYLOAD_CACHE_CNT);
}
...
int create_uac_cache(void)
{
    ...
    ret = create_cache_node_list(uac_cache->free_queue, buffer_size, cache_count);
    if (ret != 0) {
        log("create_cache_node_list failure\n");
        goto err;
    }
