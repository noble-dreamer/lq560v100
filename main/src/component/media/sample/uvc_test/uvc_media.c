...
typedef struct {
    ...
    stereo_vo_attr  vo_cfg;
    ot_eis_handle   vo_pool;
...
static ot_s32 sample_uvc_user_mode_buf_pool_creat(ot_u32 num, ot_eis_img_attr *img_attr, ot_eis_handle *pool_hdl)
{
    ot_s32 ret;
    ot_u32 block_size;
...
static ot_s32 sample_uvc_stitch_dpu_match_to_frame(ot_eis_img_frame *frame, sample_dpu_cfg *dpu_cfg, ot_avp_dpu_match_src_frame_info *match_src, ot_avp_dpu_match_dst_frame_info *match_dst)
{
    ...
    if (tgt_size > frame->attr.height * frame->buff.stride[0] + frame->attr.height * frame->buff.stride[1]) {
        sample_print("frame size not match!\n");
        return OT_FAILURE;
    }
    ...
    if (img_l->attr.width % 4 != 0) {
        sample_print("image width %d not support!\n", img_l->attr.width);
        return OT_FAILURE;
    }
    ...
    ot_u8 *dst = frame->buff.virt_addr[0];
    ...
    for (ot_u32 i = 0; i < frame->attr.height; i++) {
        ot_u8  *y = y_l + i * img_l->buff.stride[0];
        ot_u8  *d = dst + i * frame->buff.stride[0];
        memcpy(d, y, img_l->attr.width);

        y  = y_r + i * img_r->buff.stride[0];
        d += img_l->attr.width;
        memcpy(d, y, img_r->attr.width);

        ot_u16 *m = (ot_u16 *)((ot_u8 *)y_d + i * img_d->buff.stride[0]);
        d += img_r->attr.width;
        for (ot_u32 j = 0; j < img_d->attr.width / 4; j++) {
            *d++ = (*m++ >> 8) & 0xFF; // Y
            *d++ = (*m++ >> 8) & 0xFF; // Y
            *d++ = (*m++ >> 8) & 0xFF; // Y
            *d++ = (*m++ >> 8) & 0xFF; // Y
    ...
    dst = frame->buff.virt_addr[1];
    for (ot_u32 i = 0; i < frame->attr.height; i++) {
        ot_u8  *d = dst + i * frame->buff.stride[1];

        ot_u16 *c_i = i_sl + i * img_l->attr.width * 2, xi = 0, yi = 0;
        ot_u32   ss = img_l->buff.stride[1];
        for (ot_u32 j = 0; j < img_l->attr.width / 4; j++) {
            xi = ((*c_i++) >> 5) & (~0x01);
            yi = ((*c_i++) >> 6);
            *d++ = c_sl[yi * ss + xi];  // V
        ...
        for (ot_u32 j = 0; j < img_r->attr.width / 4; j++) {
            xi = ((*c_i++) >> 5) & (~0x01);
            yi = ((*c_i++) >> 6);
            *d++ = c_sr[yi * ss + xi];  // V
    ...
    dst = frame->buff.virt_addr[0];
    dst[0] = (pts >> 24) & 0xFF;
    dst[1] = (pts >> 16) & 0xFF;
    dst[2] = (pts >>  8) & 0xFF;
    dst[3] = (pts      ) & 0xFF;

    stereo_vo_flush_frame(frame);
...
static void * sample_uvc_dpu_proc(void *p)
{
    ...
    ot_eis_img_attr vo_img_attr = {
        .width         = SAMPLE_DPU_IMG_W * 3,
        .height        = SAMPLE_DPU_IMG_H,
        .pixel_fmt     = OT_EIS_IMAGE_FORMAT_YVU_422_SEMIPLANAR,
        .layout        = OT_EIS_IMAGE_LAYOUT_LINEAR,
        .bit_depth     = OT_EIS_PIXEL_BIT_DEPTH_8,
        .compress_mode = OT_EIS_IMAGE_COMPRESS_MODE_NONE,
    };
    ot_eis_img_frame vo_frame = {0};

    prctl(PR_SET_NAME, "dpu-proc", 0, 0, 0);

    stereo_vo_get_attr(&ctx->vo_cfg);
    ot_s32 ret = stereo_vo_start(&ctx->vo_cfg);
    if (ret != OT_SUCCESS) {
        sample_print("stereo_vo_start failed, ret:0x%x\n", ret);
        ctx->dpu_run = OT_FALSE;
        return NULL;
    }

    ret = sample_uvc_user_mode_buf_pool_creat(8, &vo_img_attr, &ctx->vo_pool);
    if (ret != OT_SUCCESS) {
        sample_print("create vo pool failed, ret:0x%x\n", ret);
        goto create_vo_pool_failed;
    }

    while (ctx->dpu_run) {
        ot_s32 ret = ot_avp_dpu_match_get_frame(ctx->dpu_cfg.match_grp_hdl, SAMPLE_DPU_TIME_OUT, &match_src, &match_dst);
        if (ret != OT_SUCCESS) {
            sample_print("ot_avp_dpu_match_get_frame failed, ret:0x%x\n", ret);
            continue;
        }
        
        vo_frame.attr = vo_img_attr;
        ret = stereo_vo_acquire_frame(ctx->vo_pool, &vo_frame);
        if (ret == OT_SUCCESS) {
            sample_uvc_stitch_dpu_match_to_frame(&vo_frame, &ctx->dpu_cfg, &match_src, &match_dst);
            ot_eis_vo_in_port_send_frame(ctx->vo_cfg.in_port_handle[0], &vo_frame);
            stereo_vo_release_frame(ctx->vo_pool, &vo_frame);
        }
        
        ot_avp_dpu_match_release_frame(ctx->dpu_cfg.match_grp_hdl, &match_src, &match_dst);
    }

    ot_buffer_pool_destroy(ctx->vo_pool);

create_vo_pool_failed:
    ctx->dpu_run = OT_FALSE;
    stereo_vo_stop(&ctx->vo_cfg);
...
static void * sample_uvc_get_frame_proc(void *p)
{
    ...
    if (ret != OT_SUCCESS) {
        sample_print("dpu start failed, ret:0x%x\n", ret);
        return NULL;
    }
    ctx->dpu_run = OT_TRUE;
    ret = pthread_create(&ctx->dpu_tid, NULL, sample_uvc_dpu_proc, ctx);
    if (ret != 0) {
        sample_print("start dpu proc failed, ret:0x%x\n", ret);
        goto start_dpu_proc_failed;
    }
    ...
    while (ctx->get_frm_run == OT_TRUE) {
        ...
        if ((frame_cnt++ % 150) == 0) {
            sample_print("%lld, diff pts: %lld\n", img_l.pts, (img_l.pts > img_r.pts) ? (img_l.pts - img_r.pts) : (img_r.pts - img_l.pts));
        }
...
ot_s32 sample_uvc_sc_startup(ot_u32 dev_no)
{
    ...
    for (ot_s32 i = 0; i < ctx->sns_num; i++) {
        ...
        ctx->vp_cfg[i].chn_attr[1].image_attr.pixel_fmt = OT_EIS_IMAGE_FORMAT_YVU_420_SEMIPLANAR;
    ...
    for (i_ub0 = 0; i_ub0 < ctx->sns_num; i_ub0++) {
        ...
        ret = sample_uvc_user_mode_buf_pool_creat(10, &ctx->vp_cfg[i_ub0].chn_attr[0].image_attr, &user_pool_hdl);
        if (ret != OT_SUCCESS) {
    ...
    for (i_ub1 = 0; i_ub1 < ctx->sns_num; i_ub1++) {
        ot_eis_handle user_pool_hdl = OT_NULL;
        ret = sample_uvc_user_mode_buf_pool_creat(10, &ctx->vp_cfg[i_ub1].chn_attr[1].image_attr, &user_pool_hdl);
        if (ret != OT_SUCCESS) {
            sample_print("create user buffer pool failed, ret:0x%x\n", ret);
    ...
    if (ret != 0) {
        ...
        ctx->get_frm_run = OT_FALSE;
        goto start_get_frame_proc_failed;
    }
...
{
    register_signal_handler(sample_uvc_handle_signal);

    ot_s32 ret = sample_uvc_sc_init();
    if (ret != OT_SUCCESS) {
        goto label_exit;
    }

    sample_uvc_sc_set_property(0, &(encoder_property){
        .width = 1920,
        .height = 480,
        .fps = 30,
        .format = VIDEO_IMG_FORMAT_YUYV,
        .compsite = 0,
    });

    ret = sample_uvc_sc_startup(0);

    while (gs_loop) {
        usleep(100 * 1000);
    }

    sample_uvc_sc_shutdown(0);

label_exit:
    sample_uvc_sc_deinit();
    return 0;
}
