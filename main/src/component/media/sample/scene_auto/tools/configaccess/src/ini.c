...
void str_to_lower(char *input_str)
{
    ...
    while (input_str[res_len] != '\0' && res_len < str_len) {
...
int expand_space_of_dict(ini_dictionary *ini_info_dict)
{
    ...
    if (ret0 != OT_SUCCESS || ret1 != OT_SUCCESS || ret2 != OT_SUCCESS) {
        free(new_dict_val);
        free(new_dict_key);
        free(new_dict_hash);
    }
...
static file_line_status process_line(const char *fline, char *fsection, char *fkey, char *fvalue, int buff_size)
{
...
static void process_line_status(ini_file_ctx *ctx, ini_dictionary *ini_info_dict)
{
    switch (ctx->line_sta) {
...
static int alloc_ctx_and_open_file(ini_file_ctx **ctx, ini_dictionary **ini_info_dict, const char *ini_file_name)
{
    ...
    *ctx = (ini_file_ctx *)calloc(sizeof(ini_file_ctx), 1);
    if (*ctx == NULL) {
        printf("malloc file context %lu fail\n", sizeof(ini_file_ctx));
        return -1;
    }
...
int ini_get_num_of_keys(const ini_dictionary *ini_info_dict, const char *section_name)
{
    ...
    while (section_name[i] != '\0' && i < MAXSIZE) {
