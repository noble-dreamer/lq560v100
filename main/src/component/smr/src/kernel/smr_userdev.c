...
static int smr_flush_dcache_mmb_dirty(struct dirty_area *p_area)
{
    if (p_area == NULL) {
        return -EINVAL;
    }
...
static int smr_userdev_open(struct inode *inode, struct file *file)
{
    struct smr_userdev_info *pmu = NULL;

    pmu = osal_kmalloc(sizeof(*pmu), OSAL_GFP_KERNEL);
    if (pmu == NULL) {
        smr_err_trace("alloc smr_userdev_info failed!\n");
        return -ENOMEM;
    }
...
{
    struct smr_userdev_info *pmu = file->private_data;
    struct mmb_node *mmb_node = NULL;
    ot_mmb_t *mmb = NULL;
    ot_smr_alloc_para_in para_in = {0};

    para_in.smr_name = pmi->smr_name;
    ...
    if (mmb == NULL) {
        smr_err_trace("ot_mmb_alloc(%s, %lu, 0x%lx, %lu, %s) failed!\n",
            pmi->mmb_name, (unsigned long)pmi->size, (unsigned long)pmi->align, pmi->gfp, pmi->smr_name);
        return -ENOMEM;
    }
    ...
    if (mmb_node == NULL) {
        ot_mmb_free(mmb);
        smr_err_trace("alloc mmb_node failed!\n");
        return -ENOMEM;
    }
...
static struct mmb_node *get_mmb_node(unsigned long addr, const struct smr_userdev_info *pmu)
{
...
static struct mmb_node *get_mmb_node_safe(unsigned long addr, const struct smr_userdev_info *pmu)
{
    struct mmb_node *p = NULL;

    p = get_mmb_node(addr, pmu);
    if (p == NULL) {
        smr_err_trace("mmb(0x%08lX) not found!\n", addr);
        return NULL;
    }
    return p;
}

static int smr_usrdev_mmb_free(struct mmb_node *p)
{
    int ret;
...
static int ioctl_mmb_free(struct file *file, struct mmb_info *pmi)
{
    ...
    if (p->mmb_info.delayed_free) {
        smr_warn_trace("mmb<%s> is delayed_free, can not free again!\n", p->mmb->name);
        return -EBUSY;
    }

    if ((p->map_ref > 0) || (p->mmb_ref > 0)) {
        smr_warn_trace("mmb<%s> is still in use!\n", p->mmb->name);
        p->mmb_info.delayed_free = 1;
        return -EBUSY;
    }
    return smr_usrdev_mmb_free(p);
}
...
static int ioctl_mmb_user_remap(struct file *file, struct mmb_info *pmi, int cached)
{
    ...
    if (p->mmb_info.mapped && (p->map_ref > 0)) {
        if (cached != p->mmb_info.map_cached) {
            smr_err_trace("mmb<%s> already mapped as %s, cannot remap as %s.\n", p->mmb->name,
    ...
    if (IS_ERR_VALUE((uintptr_t)addr)) {
        smr_err_trace("vm_mmap(file,0,%lu,0x%08lX,0x%08lX,0x%08lX) return 0x%08lX\n", len, prot, flags, pgoff, addr);
        return addr;
    }
...
static int ioctl_mmb_user_unmap_with_cache(const struct mmb_node *p, unsigned long addr, unsigned long len)
{
    ...
    if (ot_vma_check(addr, addr + len)) {
        smr_err_trace("mmb<%s> vma is invalid.\n", p->mmb->name);
        smr_map_up(mm);
        return -EPERM;
    }
...
static int ioctl_mmb_user_unmap(struct file *file, struct mmb_info *pmi)
{
    ...
    if (p->mmb_info.mapped == NULL) {
        smr_warn_trace("mmb(0x%lx) isn't user-mapped!\n", (unsigned long)p->mmb_info.phys_addr);
        pmi->mapped = NULL;
        return -EIO;
    }

    if (!((p->map_ref > 0) && (p->mmb_ref > 0))) {
        smr_err_trace("mmb<%s> has invalid refer: map_ref=%d, mmb_ref=%d.\n",
...
static int ioctl_mmb_virt2phys(struct file *file, struct mmb_info *pmi)
{
    ...
    if (ot_mmb_getby_phys_2(phys, &offset) == NULL) {
        smr_err_trace("Not smr alloc memory[0x%lx 0x%lx]! 0x%lx\n", virt, phys, offset);
        return -EINVAL;
    }
    pmi->phys_addr = phys;
    return ret;
...
int ioctl_mmb_sys_flush_cache(__phys_addr_type__ phy_addr, void *vir_addr, unsigned int size)
{
    ...
    if (trans_phy == 0) {
        smr_err_trace("start virtual address %pK is err.\n", vir_addr);
        return -1;
    }

    if ((trans_phy & 0xFFFFFFFFFFFFFFFEULL) != phy_addr) {
        smr_err_trace("trans_phy 0x%lx and phy_addr 0x%lx are not equal!\n", trans_phy, phy_addr);
        return -1;
    }
    ...
    if (trans_phy == 0) {
        smr_err_trace("end virtual address 0x%lx is err.\n", end_vir_addr);
        return -1;
    }

    end_phy_addr = phy_addr + size - CACHE_LINE_SIZE;
    if ((trans_phy & 0xFFFFFFFFFFFFFFFEULL) != end_phy_addr) {
        smr_err_trace("trans_phy 0x%lx and end_phy_addr 0x%lx are not equal!\n", trans_phy, end_phy_addr);
        return -1;
    }
...
int ioctl_mmb_check_smr_phy_addr(unsigned long long phy_addr, unsigned int len)
{
    /* if address in smr of current system */
    if (ot_is_phys_in_smr((unsigned long)phy_addr, len)) {
        /* if in other system */
        if (ot_map_smr_check_phys((unsigned long)phy_addr, len)) {
            return -1;
        }
    }
    return 0;
...
static int smr_userdev_ioctl_m(struct file *file, unsigned int cmd, struct mmb_info *pmi)
{
    switch (_IOC_NR(cmd)) {
        ...
        case _IOC_NR(IOC_MMB_BASE_CHECK_ADDR):
            return ioctl_mmb_check_smr_phy_addr(pmi->phys_addr, pmi->size);
        default:
            smr_err_trace("invalid ioctl cmd = %08X\n", cmd);
            return -EINVAL;
    }
}

static int smr_userdev_ioctl_of_m(struct file *file, unsigned int cmd, unsigned long arg)
{
    struct mmb_info mi = { 0 };
    int ret;

    if ((_IOC_SIZE(cmd) > sizeof(mi)) || (arg == 0)) {
        smr_err_trace("_IOC_SIZE(cmd)=%d\n", _IOC_SIZE(cmd));
        return -EINVAL;
    }
    (void)memset_s(&mi, sizeof(mi), 0, sizeof(mi));
    if (copy_from_user(&mi, (void *)(uintptr_t)arg, _IOC_SIZE(cmd))) {
        smr_err_trace("copy_from_user error.\n");
        return -EFAULT;
    }
    mi.smr_name[OT_SMR_NAME_LEN - 1] = '\0';
    mi.mmb_name[OT_MMB_NAME_LEN - 1] = '\0';
    ret = smr_userdev_ioctl_m(file, cmd, &mi);
    if (!ret && (cmd & IOC_OUT)) {
        if (copy_to_user((void *)(uintptr_t)arg, &mi, _IOC_SIZE(cmd))) {
            smr_err_trace("copy_to_user error.\n");
            return -EFAULT;
        }
    }
    return ret;
...
static int ioctl_mmb_mem_share(const struct mmb_share_info *share_info)
{
    ...
    if (mmb == NULL) {
        smr_err_trace("invalid mem handle!\n");
        return -EINVAL;
    }

    if (mmb->kernel_only == 1) {
        media_mem_put_mmb(mmb);
        smr_err_trace("share failed, mmb used only in kernel!\n");
        return -EPERM;
    }
    ...
    if (mmb->check_pid != 1) {
        media_mem_put_mmb(mmb);
        smr_err_trace("buffer is already share all!\n");
        return -EPERM;
    }
    /* pid[0] can share */
    if (osal_get_current_tgid() != mmb->pid[0]) {
        media_mem_put_mmb(mmb);
        smr_err_trace("only process that alloc buffer can share!\n");
        return -EPERM;
    }
    if (share_info->shared_pid == mmb->pid[0]) {
        media_mem_put_mmb(mmb);
        smr_err_trace("cannot share to process that alloc buffer!\n");
        return -EPERM;
    }
    if (mmb->pid_num >= OT_MMB_MAX_PID_NUM) {
        media_mem_put_mmb(mmb);
        smr_err_trace("share failed, max pid_num is %d!\n", OT_MMB_MAX_PID_NUM);
        return -EPERM;
    }
...
static int ioctl_mmb_mem_unshare(const struct mmb_share_info *share_info)
{
    ...
    if (mmb->kernel_only == 1) {
        media_mem_put_mmb(mmb);
        smr_err_trace("unshare failed, mmb used only in kernel!\n");
        return -EPERM;
    }
    ...
    if (pos < 0) {
        media_mem_put_mmb(mmb);
        smr_err_trace("mmb is not shared with pid(%d)!\n", share_info->shared_pid);
        return -EPERM;
    }
    if (share_info->shared_pid == mmb->pid[0]) {
        media_mem_put_mmb(mmb);
        smr_err_trace("unshare failed, pid(%d) is the process that alloc buffer!\n", share_info->shared_pid);
        return -EPERM;
    }
    cur_pid = osal_get_current_tgid();
    if (cur_pid != mmb->pid[0] && share_info->shared_pid != cur_pid) {
        media_mem_put_mmb(mmb);
        smr_err_trace("process cannot unshare with pid(%d)!\n", share_info->shared_pid);
        return -EPERM;
    }
    /* unshare */
...
static int ioctl_mmb_mem_share_all(const struct mmb_share_info *share_info)
{
    ...
    if (mmb->kernel_only == 1) {
        media_mem_put_mmb(mmb);
        smr_err_trace("share all failed, mmb used only in kernel!\n");
        return -EPERM;
    }
    ...
    if (osal_get_current_tgid() != mmb->pid[0]) {
        media_mem_put_mmb(mmb);
        smr_err_trace("only process that alloc buffer can share all!\n");
        return -EPERM;
    }
    /* share all */
    mmb->check_pid = 0;
...
static int ioctl_mmb_mem_unshare_all(const struct mmb_share_info *share_info)
{
    ...
    if (mmb->kernel_only == 1) {
        media_mem_put_mmb(mmb);
        smr_err_trace("unshare all failed, mmb used only in kernel!\n");
        return -EPERM;
    }
    ...
    if (osal_get_current_tgid() != mmb->pid[0]) {
        media_mem_put_mmb(mmb);
        smr_err_trace("only process that alloc buffer can unshare all!\n");
        return -EPERM;
    }
    /* unshare all */
    mmb->check_pid = 1;
...
static int ioctl_mmb_get_sys_mem_by_virt(struct mmb_share_info *share_info)
{
    ...
    if (phys_addr == 0) {
        smr_err_trace("invalid virt_addr!\n");
        return -EINVAL;
    }

    mmb = media_mem_get_mmb_by_phys(phys_addr);
    if (mmb == NULL) {
        smr_err_trace("not smr alloc memory[0x%lx 0x%lx]!\n", virt_addr, phys_addr);
        return -EINVAL;
    }
...
static int ioctl_mmb_get_sys_mem_by_phys(struct mmb_share_info *share_info)
{
    ...
    mmb = media_mem_get_mmb_by_phys(phys_addr);
    if (mmb == NULL) {
        smr_err_trace("not smr alloc memory[0x%lx]!\n", phys_addr);
        return -EINVAL;
    }

    if (ot_mmb_check_mem_share(mmb) < 0) {
        media_mem_put_mmb(mmb);
        smr_err_trace("mem share check failed!\n");
        return -EPERM;
    }
...
static int smr_userdev_ioctl_s(struct file *file, unsigned int cmd, struct mmb_share_info *share_info)
{
    switch (_IOC_NR(cmd)) {
...
static int smr_userdev_ioctl_of_s(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    ret = smr_userdev_ioctl_s(file, cmd, &share_info);
    if (!ret && (cmd & IOC_OUT)) {
...
static int smr_userdev_check_dirty_area_range(const struct dirty_area *area,
    const struct smr_userdev_info *pmu)
{
    ot_mmb_t *mmb = NULL;
    struct mmb_node *mmb_node = NULL;
    ...
    if (mmb == NULL) {
        smr_err_trace("dirty_phys_addr=0x%lx\n", (unsigned long)area->dirty_phys_start);
        return -EFAULT;
    }
    ...
    if ((uintptr_t)area->dirty_virt_start != (uintptr_t)mmb_node->mmb_info.mapped + offset) {
        media_mem_put_mmb(mmb);
        smr_warn_trace("dirty_virt_start addr was not consistent with dirty_phys_start addr!\n");
        return -EFAULT;
    }
    if (area->dirty_phys_start + area->dirty_size > mmb->phys_addr + mmb->length) {
        media_mem_put_mmb(mmb);
        smr_warn_trace("dirty area overflow!\n");
        return -EFAULT;
    }
    media_mem_put_mmb(mmb);
    return 0;
...
static int smr_userdev_ioctl_of_d(unsigned int cmd, unsigned long arg, const struct smr_userdev_info *pmu)
{
    ...
    if (copy_from_user(&area, (void *)(uintptr_t)arg, _IOC_SIZE(cmd))) {
        smr_warn_trace("copy_from_user error.\n");
        return -EFAULT;
    }

    ret = smr_userdev_check_dirty_area_range(&area, pmu);
    if (ret != 0) {
        return ret;
    }
    ...
    if (ot_vma_check((uintptr_t)area.dirty_virt_start, (uintptr_t)area.dirty_virt_start + area.dirty_size)) {
        smr_warn_trace("dirty area[0x%lx,0x%lx] overflow!\n", (unsigned long)(uintptr_t)area.dirty_virt_start,
    ...
    smr_flush_dcache_mmb_dirty(&area);
    smr_map_up(mm);
    return 0;
}

static long smr_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    int ret;
    struct smr_userdev_info *pmu = file->private_data;
    ...
    if (_IOC_TYPE(cmd) == SMR_MMB_IOC_MAGIC) {
        ret = smr_userdev_ioctl_of_m(file, cmd, arg);
    } else if (_IOC_TYPE(cmd) == SMR_MMB_SHARE_IOC_MAGIC) {
        ret = smr_userdev_ioctl_of_s(file, cmd, arg);
    } else if (_IOC_TYPE(cmd) == SMR_MMB_DCACHE_DIRTY_IOC_MAGIC) {
        ret = smr_userdev_ioctl_of_d(cmd, arg, pmu);
    } else {
        ret = -EINVAL;
    }
...
#ifdef CONFIG_COMPAT
static long compat_smr_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    return smr_userdev_ioctl(file, cmd, (unsigned long)(uintptr_t)compat_ptr(arg));
}
#endif

static void smr_userdev_mmap_page_valid(struct vm_area_struct *vma)
{
    ...
    while (size) {
        if (pfn_valid(pfn)) {
            if (vm_insert_page(vma, start, pfn_to_page(pfn))) {
                smr_err_trace("insert page failed.\n");
                break;
            }
        } else {
            smr_err_trace("vm map failed for phy address(0x%lx)\n", (unsigned long)__pfn_to_phys(pfn));
        }
...
static int smr_userdev_mmap_page_invalid(struct vm_area_struct *vma)
{
    size_t size = vma->vm_end - vma->vm_start;
    if (size == 0) {
        return -EPERM;
...
static int smr_userdev_do_mmap(struct vm_area_struct *vma, int mmb_mapped_k, int mmb_cached_k, int mmb_cached_u)
{
    if (mmb_cached_u == 0) {
        /* mapping as non-cacheable */
        if ((mmb_mapped_k != 0) && (mmb_cached_k != 0)) {
            /* mmb has already been mapped and mapped as cacheable */
            smr_err_trace("memory mmap[0x%lx, 0x%lx] prot as non-cacheable conflit\n",
        ...
        if ((mmb_mapped_k != 0) && (mmb_cached_k == 0)) {
            ...
            smr_err_trace("memory mmap[0x%lx, 0x%lx] prot as cacheable conflit\n",
                vma->vm_pgoff, vma->vm_end - vma->vm_start);
            return -EPERM;
        }
#ifdef CONFIG_64BIT
    ...
    if (pfn_valid(vma->vm_pgoff)) {
        smr_userdev_mmap_page_valid(vma);
        return 0;
    }
    return smr_userdev_mmap_page_invalid(vma);
}

int smr_userdev_mmap(struct file *file, struct vm_area_struct *vma)
{
    struct mmb_node *p = NULL;
    unsigned long offset;
    ot_mmb_t *mmb = NULL;
    ...
    p = get_mmb_node(offset, (struct smr_userdev_info *)file->private_data);
    mmb = media_mem_get_mmb_by_phys_and_size(offset, vma->vm_end - vma->vm_start);
    if (p == NULL) {
        if (mmb == NULL) {
            /*
             * Allow mmap SMR allocated by other co-working OS, but we don't have and
             ...
             */
            if (ot_map_smr_check_phys(offset, vma->vm_end - vma->vm_start)) {
                return -EPERM;
            }
        } else {
            if (ot_mmb_check_mem_share(mmb) < 0) {
        ...
        /*
         * If file opened as O_SYNC, mmap the smr area as write-combine, which
         * is also non-cacheable, otherwise mmap the area as normal(cacheable).
         */
        mmb_cached_u = (file->f_flags & O_SYNC) ? 0 : 1;
    } else {
        if (mmb == NULL) {
            smr_err_trace("the mmap addr must be in the mmb.\n");
            return -EPERM;
        }
        media_mem_put_mmb(mmb);
    ...
    ret = smr_userdev_do_mmap(vma, mmb_mapped_k, mmb_cached_k, mmb_cached_u);
    if (ret != 0) {
        return ret;
    }
...
static int smr_userdev_release(struct inode *inode, struct file *file)
{
    struct smr_userdev_info *pmu = file->private_data;
    struct mmb_node *p = NULL;
    ...
    osal_list_for_each_entry_safe(p, n, &pmu->list, list) {
        smr_err_trace("MMB LEAK(pid=%d): 0x%lX, %lu bytes, '%s'\n",
         ...
         */
        if (p->mmb_info.mapped != NULL) {
            smr_printk(KERN_WARNING "mmb<0x%lx> mapped to userspace 0x%pK will be unmaped!\n",
...
static struct file_operations g_smr_userdev_fops = {
    .owner = THIS_MODULE,
    .open = smr_userdev_open,
    .release = smr_userdev_release,
#ifdef CONFIG_COMPAT
    .compat_ioctl = compat_smr_userdev_ioctl,
#endif
    .unlocked_ioctl = smr_userdev_ioctl,
    .mmap = smr_userdev_mmap,
};

static struct miscdevice g_smr_userdev = {
    .minor = MISC_DYNAMIC_MINOR,
    .fops = &g_smr_userdev_fops,
    .name = "smr_userdev"
};

int __init smr_userdev_init(void)
{
    int ret;
    ret = misc_register(&g_smr_userdev);
    if (ret) {
        smr_err_trace("register smr dev failure!\n");
        return -1;
    }
...
void __exit smr_userdev_exit(void)
{
    misc_deregister(&g_smr_userdev);
}
