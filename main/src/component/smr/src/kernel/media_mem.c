...
#define SMR_SETUP_CMDLINE_LEN     256
#define SMR_ALLOCATOR_NAME_LEN    32

OSAL_LIST_HEAD(g_smr_list);
OSAL_LIST_HEAD(g_map_smr_list);

static DEFINE_SEMAPHORE(g_smr_lock);

static struct smr_allocator g_the_allocator;

static int  anony = 1;
static bool mem_process_isolation = true;
static char g_setup_zones[SMR_SETUP_CMDLINE_LEN] = {'\0'};
static char g_mmap_zones[SMR_SETUP_CMDLINE_LEN] = {'\0'};
static char g_setup_allocator[SMR_ALLOCATOR_NAME_LEN] = "ot"; /* default setting */

#ifndef MODULE
osal_setup_num_param(anony, anony);
osal_setup_num_param(mem_process_isolation, mem_process_isolation);
osal_setup_str_param(smr_allocator, g_setup_allocator);
osal_setup_str_param(smr_zones, g_setup_zones);
osal_setup_str_param(map_smr, g_mmap_zones);
#else
module_param(anony, int, S_IRUGO);
module_param(mem_process_isolation, bool, S_IRUGO);
module_param_string(smr, g_setup_zones, SMR_SETUP_CMDLINE_LEN, 0600);
module_param_string(map_smr, g_mmap_zones, SMR_SETUP_CMDLINE_LEN, 0600);
module_param_string(smr_allocator, g_setup_allocator, SMR_ALLOCATOR_NAME_LEN, 0600);
MODULE_PARM_DESC(smr, "smr_allocator=allocator smr=name,0,start,size:[others] map_smr=start,size:[others]");
#endif
...
typedef void (*smr_destructor)(const void *);
ot_smr_t *ot_smr_create(const char *name, unsigned long gfp, unsigned long phys_start, unsigned long nbytes)
{
    ot_smr_t *p = NULL;

    if (name == NULL) {
        smr_err_trace("name should not be NULL!");
        return NULL;
    }

    p = osal_kmalloc(sizeof(ot_smr_t) + 1, OSAL_GFP_KERNEL);
    if (p == NULL) {
        smr_err_trace("System OOM!\n");
        return NULL;
    }

    (void)memset_s(p, sizeof(ot_smr_t) + 1, 0, sizeof(ot_smr_t) + 1);

    if (strncpy_s(p->name, OT_SMR_NAME_LEN, name, OT_SMR_NAME_LEN - 1) != EOK) {
    ...
    p->destructor = (smr_destructor)osal_kfree;

    return p;
}
EXPORT_SYMBOL(ot_smr_create);

int ot_smr_destroy(const ot_smr_t *zone)
{
    if (zone == NULL) {
        return -1;
    }
...
EXPORT_SYMBOL(ot_smr_destroy);

static int smr_register_check(const ot_smr_t *zone)
{
    ot_smr_t *p = NULL;
    ...
    if (!((new_start >= __pa((uintptr_t)high_memory)) ||
        ...
        smr_err_trace("Conflict SMR:\n"
            OT_SMR_FMT_S "\n"
            "SMR conflict to kernel memory (0x%08lX, 0x%08lX)\n", ot_smr_fmt_arg(zone),
            (unsigned long)PHYS_OFFSET, (unsigned long)(__pa((uintptr_t)high_memory) - 1));
        return -1;
    }

    osal_list_for_each_entry(p, &g_smr_list, list) {
        ...
        smr_err_trace("Conflict SMR:\n"
            "SMR new:   " OT_SMR_FMT_S "\n"
            "SMR exist: " OT_SMR_FMT_S "\n"
            "Add new SMR failed!\n", ot_smr_fmt_arg(zone), ot_smr_fmt_arg(p));
        return -1;
    }
    return 0;
}
...
int ot_smr_register(ot_smr_t *zone)
{
    ...
    smr_debug_info(OT_SMR_FMT_S, ot_smr_fmt_arg(zone));

    down(&g_smr_lock);
    if (strcmp(g_setup_allocator, "ot") == 0) {
        ret = smr_register_check(zone);
        if (ret) {
            up(&g_smr_lock);
            return ret;
        }
    ...
    osal_list_add(&zone->list, &g_smr_list);
    up(&g_smr_lock);
    return 0;
}
...
int ot_smr_unregister(ot_smr_t *zone)
{
    ...
    osal_list_for_each_entry(p, &zone->mmb_list, list) {
        smr_warn_trace("MMB Lost: " OT_MMB_FMT_S "\n", ot_mmb_fmt_arg(p));
        losts++;
    }

    if (losts) {
        smr_err_trace("%d mmbs not free, smr<%s> can not be unregistered!\n", losts, zone->name);
        up(&g_smr_lock);
        return -1;
    }
...
ot_mmb_t *ot_mmb_alloc(const ot_smr_alloc_para_in *para_in)
{
    ot_mmb_t *mmb = NULL;
...
int ot_mmb_get(ot_mmb_t *mmb)
{
    ...
    if (mmb->flags & OT_MMB_RELEASED) {
        smr_warn_trace("mmb<%s> is released!\n", mmb->name);
    }
    ref = ++mmb->phy_ref;
    up(&g_smr_lock);
    return ref;
...
int ot_mmb_free(ot_mmb_t *mmb)
{
    ...
    smr_debug_info(OT_MMB_FMT_S, ot_mmb_fmt_arg(mmb));
    down(&g_smr_lock);

    if (mmb->flags & OT_MMB_RELEASED) {
        smr_warn_trace("mmb<%s> has been released, but is still in use!\n", mmb->name);
        up(&g_smr_lock);
        return 0;
    }

    if (mmb->phy_ref > 0) {
        smr_warn_trace("free mmb<%s> delayed for which ref-count is %d!\n", mmb->name, mmb->map_ref);
    ...
    if (mmb->flags & OT_MMB_MAP2KERN) {
        smr_warn_trace("free mmb<%s> delayed for which is kernel-mapped with map_ref %d!\n",
            mmb->name, mmb->map_ref);
        mmb->flags |= OT_MMB_RELEASED;
        up(&g_smr_lock);
        return 0;
...
ot_mmb_t *ot_mmb_getby_phys(unsigned long addr)
{
    ...
    down(&g_smr_lock);
    osal_list_for_each_entry(smr, &g_smr_list, list) {
        osal_list_for_each_entry(mmb, &smr->mmb_list, list) {
...
unsigned long usr_virt_to_phys(unsigned long virt)
{
    ...
    if (virt & 0x3) {
        smr_err_trace("invalid virt addr 0x%08lx[not 4 bytes align]\n", virt);
        return 0;
    }

    if (virt >= PAGE_OFFSET) {
        smr_err_trace("invalid user space virt addr 0x%08lx\n", virt);
        return 0;
    }

    pgd = pgd_offset(current->mm, virt);
    if (pgd_none(*pgd)) {
        smr_err_trace("not mapped in pgd!\n");
        return 0;
    }

    p4d = p4d_offset(pgd, virt);
    if (p4d_none(*p4d)) {
        smr_err_trace("not mapped in p4d!\n");
        return 0;
    }
    pud = pud_offset(p4d, virt);
    if (pud_none(*pud)) {
        smr_err_trace("not mapped in pud!\n");
        return 0;
    }

    pmd = pmd_offset(pud, virt);
    if (pmd_none(*pmd)) {
        smr_err_trace("not mapped in pmd!\n");
        return 0;
    }

    pte = pte_offset_map(pmd, virt);
    if (pte_none(*pte)) {
        smr_err_trace("not mapped in pte!\n");
        pte_unmap(pte);
        return 0;
    }
...
ot_smr_t *ot_smr_find(unsigned long gfp, const char *smr_name)
{
    ot_smr_t *p = NULL;

    down(&g_smr_lock);
    osal_list_for_each_entry(p, ot_smr_get_smr_list(), list) {
        if (smr_default_filter(p, gfp, smr_name) != 0) {
            continue;
        }
        up(&g_smr_lock);
        return p;
...
EXPORT_SYMBOL(ot_smr_find);

unsigned long ot_smr_get_phys(const char *zone_name)
{
    ot_smr_t *zone = NULL;

    zone = ot_smr_find(0, zone_name);
    if (zone != NULL) {
...
static int map_smr_register_check(const ot_smr_t *zone)
{
    ot_smr_t *p = NULL;
    ...
    if (new_end <= new_start) {
        smr_err_trace("Invalid map_smr(0x%lX,%lu)!", new_start, zone->nbytes);
        return -1;
    }

    osal_list_for_each_entry(p, &g_map_smr_list, list) {
        unsigned long start, end;
        start = p->phys_start;
        end   = p->phys_start + p->nbytes;
...
int ot_map_smr_register(ot_smr_t *zone)
{
    int ret;
    ...
    ret = map_smr_register_check(zone);
    if (ret) {
        up(&g_smr_lock);
        return ret;
    }
    ...
    OSAL_INIT_LIST_HEAD(&zone->mmb_list);
    osal_list_add(&zone->list, &g_map_smr_list);

    up(&g_smr_lock);
    return 0;
}

int ot_map_smr_unregister(ot_smr_t *zone)
{
    int losts = 0;
    ot_mmb_t *p = NULL;
...
static int map_smr_init(char *s)
{
    ot_smr_t *zone = NULL;
    char *line = NULL;
    ...
    while ((line = strsep(&s, ":")) != NULL) {
        int i;
        char *argv[2]; /* 2: map smr has two arguments */
        ...
        if (i == 2) { /* 2: had parse two args */
            zone = ot_smr_create("null", 0, 0, 0);
            if (zone == NULL) {
                continue;
            }
            zone->phys_start = smr_strtoul_ex(argv[0], NULL, 0);
            zone->nbytes = smr_strtoul_ex(argv[1], NULL, 0);
        } else {
            smr_err_trace("error parameters\n");
            return -EINVAL;
        }

        if (ot_map_smr_register(zone)) {
...
static void map_smr_exit(void)
{
    ot_smr_t *ptr = NULL;
    struct osal_list_head *p = NULL;
    struct osal_list_head *n = NULL;

    osal_list_for_each_safe(p, n, &g_map_smr_list) {
        ptr = osal_list_entry(p, ot_smr_t, list);
        smr_warn_trace("SMR force removed: " OT_SMR_FMT_S "\n", ot_smr_fmt_arg(ptr));
        ot_map_smr_unregister(ptr);
        ot_smr_destroy(ptr);
    }
}

int ot_map_smr_check_phys(unsigned long addr_start, unsigned long addr_len)
{
    ot_smr_t *p = NULL;
    ...
    smr_err_trace("invalid mmap addr: 0x%lx-0x%lx\n", addr_start, addr_end);
    return -1;
}
EXPORT_SYMBOL(ot_map_smr_check_phys);
...
int ot_mmb_check_mem_share_with_pid(const ot_mmb_t *mmb, int pid)
{
    ...
    if (mmb->kernel_only == 1) {
        smr_err_trace("mmb used only in kernel!\n");
        return -EPERM;
    }
    if (mmb->check_pid == 0) {
        /* default share */
...
int ot_smr_check_phys_addr(unsigned long phys_addr, unsigned long size)
{
    ot_mmb_t *mmb = NULL;
    int ret;

    /* if allocated by smr of current system */
    mmb = media_mem_get_mmb_by_phys_and_size(phys_addr, size);
    if (mmb == NULL) {
        /* if in other system */
        if (ot_map_smr_check_phys(phys_addr, size) < 0) {
            return -EINVAL;
        }
        return 0;
    }
...
EXPORT_SYMBOL(ot_smr_check_phys_addr);

int ot_smr_get_mem_process_isolation(void)
{
    return mem_process_isolation;
}
EXPORT_SYMBOL(ot_smr_get_mem_process_isolation);
...
int ot_vma_check(unsigned long vm_start, unsigned long vm_end)
{
    ...
    if (pvma1 == NULL) {
        smr_err_trace("pvma1 is null\n");
        return -1;
    }

    pvma2 = find_vma(current->mm, vm_end - 1);
    if (pvma2 == NULL) {
        smr_err_trace("pvma2 is null\n");
        return -1;
    }

    if (pvma1 != pvma2) {
        smr_err_trace("pvma1:[0x%lx,0x%lx) and pvma2:[0x%lx,0x%lx) are not equal\n",
            pvma1->vm_start, pvma1->vm_end, pvma2->vm_start, pvma2->vm_end);
        return -1;
    }

    if (!(pvma1->vm_flags & VM_WRITE)) {
        smr_err_trace("invalid vma flag:0x%lx\n", pvma1->vm_flags);
        return -1;
    }

    if (pvma1->vm_start > vm_start) {
        smr_err_trace("cannot find corresponding vma, vm[%lx, %lx], user range[%lx,%lx]\n",
...
int ot_is_phys_in_smr(unsigned long addr_start, unsigned long addr_len)
{
    ot_smr_t *p = NULL;
    unsigned long addr_end = addr_start + addr_len;
    unsigned long temp_start, temp_end;
...
EXPORT_SYMBOL(ot_is_phys_in_smr);
...
int smr_seq_show(struct osal_proc_dir_entry *sfile)
{
    ot_smr_t *p = NULL;
    ...
    unsigned int smr_total_size = 0;

    osal_seq_printf(sfile->seqfile, " anony=%d smr_allocator=%s mem_process_isolation=%d\n\n",
        anony, g_setup_allocator, mem_process_isolation);

    down(&g_smr_lock);
    if (!osal_list_empty(&g_map_smr_list)) {
        osal_seq_printf(sfile->seqfile, " map_smr_zone:");
        osal_list_for_each_entry(p, &g_map_smr_list, list) {
    ...
    osal_list_for_each_entry(p, &g_smr_list, list) {
        ...
        osal_seq_printf(sfile->seqfile, "+---ZONE: " OT_SMR_FMT_S "\n", ot_smr_fmt_arg(p));
        smr_total_size += p->nbytes / 1024; /* 1024: 1KByte = 1024Byte */
        ++zone_number;

        osal_list_for_each_entry(mmb, &p->mmb_list, list) {
            if (temp_mmb != NULL && ((mmb)->phys_addr > smr_grain_align((temp_mmb)->phys_addr + (temp_mmb)->length))) {
    ...
    if (smr_total_size != 0) {
        free_size = smr_total_size - used_size;
        osal_seq_printf(sfile->seqfile, "\n---SMR_USE_INFO:\n total size=%dKB(%dMB),"
            "used=%dKB(%dMB + %dKB),remain=%dKB(%dMB + %dKB),"
            "zone_number=%d,block_number=%d\n",
            smr_total_size, smr_total_size / 1024, /* 1024: 1MByte = 1024KByte */
...
static int __init media_mem_proc_init(void)
{
    ...
    proc = osal_create_proc_entry(SMR_MEM_NAME, NULL);
    if (proc == NULL) {
        smr_err_trace("Create smr proc fail!\n");
        return -1;
    }
    proc->read = smr_seq_show;
    return 0;
}

static void __exit media_mem_proc_exit(void)
{
    osal_remove_proc_entry(SMR_MEM_NAME, NULL);
}
...
{
    if (anony != 1) {
        smr_err_trace("The module param \"anony\" should only be 1 which is %d\n", anony);
        return -1;
    }
    return 0;
}

static void smr_exit_check(void)
{
    ot_smr_t *ptr = NULL;
    struct osal_list_head *p = NULL;
    struct osal_list_head *n = NULL;

    osal_list_for_each_safe(p, n, &g_smr_list) {
        ptr = osal_list_entry(p, ot_smr_t, list);
        smr_warn_trace("SMR force removed: " OT_SMR_FMT_S "\n", ot_smr_fmt_arg(ptr));
        ot_smr_unregister(ptr);
        ot_smr_destroy(ptr);
    }
}
...
int __init media_mem_init(void)
{
    ...
    smr_printk(KERN_INFO "Media Memory Zone Manager\n");
    ...
    if (strcmp(g_setup_allocator, "ot") == 0) {
        ...
        smr_err_trace("The module param \"g_setup_allocator\" should be \"cma\" or \"ot\", which is \"%s\"\n",
            g_setup_allocator);
        smr_exit_check();
        return -EINVAL;
    }
