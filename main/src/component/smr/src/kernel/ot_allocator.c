    ...
    unsigned long *fixed_start, unsigned long *fixed_len, unsigned long len)
{
    ...
    if ((blank_len < *fixed_len) && (blank_len >= len)) {
        *fixed_len = blank_len;
        *fixed_start = start;
        smr_debug_info("start=0x%08lX, len=%luKB\n", *fixed_start, *fixed_len / SZ_1K);
        ret = 1;
    }
    return ret;
}
...
static unsigned long find_fixed_region(unsigned long *region_len,
    ot_smr_t *smr, unsigned long size, unsigned long align)
{
    ...
    align = smr_grain_align(align);
    if (align == 0) {
        align = SMR_GRAIN;
    }
    start = smr_align2(smr->phys_start, align);
    len = smr_grain_align(size);

    osal_list_for_each_entry(p, &smr->mmb_list, list) {
        ot_mmb_t *next = NULL;
        smr_debug_info("p->phys_addr=0x%08lX p->length = %luKB \t", p->phys_addr, p->length / SZ_1K);
        next = osal_list_entry(p->list.next, typeof(*p), list);
        smr_debug_info(",next = 0x%08lX\n\n", next->phys_addr);
        /*
         * if p is the first entry or not.
         */
        if (osal_list_first_entry(&smr->mmb_list, typeof(*p), list) == p) {
            (void)calculate_fixed_region(p->phys_addr, start, &fixed_start, &fixed_len, len);
        }
        start = smr_align2((p->phys_addr + p->length), align);
        /* if aglin is larger than smr->nbytes, it would trigger the BUG_ON */
        /* if we have to alloc after the last node.  */
        if (osal_list_is_last(&p->list, &smr->mmb_list)) {
            if (calculate_fixed_region(smr->phys_start + smr->nbytes, start, &fixed_start, &fixed_len, len) == 1) {
                break;
            }
        ...
        if ((start + len) > next->phys_addr) {
            smr_debug_info("start=0x%08lX, len=%lu, next=0x%08lX\n", start, len, next->phys_addr);
            continue;
        }
    ...
    if ((smr_grain_align(start + len) <= (smr->phys_start + smr->nbytes)) &&
        (start >= smr->phys_start) && (start < (smr->phys_start + smr->nbytes))) {
        fixed_start = start;
    } else {
        fixed_start = 0;
    }
...
int do_mmb_alloc(ot_mmb_t *mmb)
{
    ...
    osal_list_for_each_entry(p, &mmb->zone->mmb_list, list) {
        ...
        if (mmb->phys_addr == p->phys_addr) {
            smr_err_trace("media-mem allocator bad in %s!", mmb->zone->name);
            return -1;
        }
    }
...
static ot_mmb_t *allocator_mmb_alloc(const ot_smr_alloc_para_in *para_in)
{
    ...
    smr_debug_info("size=%luKB, align=%lu", size / SZ_1K, align);

    osal_list_for_each_entry(smr, ot_smr_get_smr_list(), list) {
        if (smr_default_filter(smr, gfp, smr_name) != 0) {
            continue;
        }
        start = find_fixed_region(&region_len, smr, size, align);
    ...
    return mmb_real_alloc(para_in, fixed_smr, fixed_start, size);
}
...
static void *allocator_mmb_map2kern(ot_mmb_t *mmb, int cached)
{
    ...
    if (mmb->flags & OT_MMB_MAP2KERN) {
        if ((!!cached * OT_MMB_MAP2KERN_CACHED) != (mmb->flags & OT_MMB_MAP2KERN_CACHED)) {
            smr_err_trace("mmb<%s> has been kernel-mapped as %s, can not be re-mapped as %s.", mmb->name,
...
static int allocator_init_parse_args(ot_smr_t **zone, int argc, char **argv)
{
    ...
    smr_zone->gfp        = smr_strtoul_ex(argv[1], NULL, 0); /* 1: the second args */
    smr_zone->phys_start = smr_strtoul_ex(argv[2], NULL, 0); /* 2: the third args */
    smr_zone->nbytes     = smr_strtoul_ex(argv[3], NULL, 0); /* 3: the fourth args */
    if (smr_zone->nbytes > g_ot_max_malloc_size) {
        g_ot_max_malloc_size = smr_zone->nbytes;
    }
    *zone = smr_zone;
    return 1;
}
...
static int allocator_init(char *s)
{
    ...
    while ((line = strsep(&s, ":")) != NULL) {
        ...
        if ((zone->phys_start > phys_end) && (phys_end != 0)) {
            smr_err_trace("SMR: parameter is not correct! Address exceeds 0xFFFFFFFF\n");
            ot_smr_unregister(zone);
            ot_smr_destroy(zone);
            return -1;
        }
