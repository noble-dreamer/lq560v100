...
#else
    if (ot_smr_alloc(&alloc_attr, &phys_addr) != OT_SUCCESS) {
        return OT_FAILURE;
    }
    if (ot_smr_mmap(phys_addr, alloc_attr.len, OT_FALSE, (ot_void **)&buf->virt_addr) != OT_SUCCESS) {
        return OT_FAILURE;
    }
#endif
...
#else
    ot_smr_flush_cache((ot_phys_addr)buf->phys_addr, size);
#endif
...
{
    if (buf->virt_addr != 0) {
#ifdef OT_AVP_CVE_V200
        ot_smr_munmap((ot_void *)buf->virt_addr);
#endif
...
#else
    ot_smr_flush_cache((ot_phys_addr)buf->phys_addr, buf->capacity);
#endif
    return OT_AVP_CVE_SUCCESS;
}
...
ot_s32 ot_avp_cve_create_single_planar_img(ot_avp_cve_img *img)
{
    ...
    if (ot_smr_mmap(phys_addr, alloc_attr.len, OT_FALSE, (ot_void **)&img->virt_addr[0]) != OT_SUCCESS) {
        return OT_FAILURE;
    }
#endif
...
// 此处V100和V200 img stride 结构体填充有差异
...
ot_s32 ot_avp_cve_create_bitmap_img(ot_avp_cve_img *img)
{
    ...
    img->phys_addr[2] = 0;
    img->capacity = img_size;
    printf("[ot_avp_cve_create_bitmap_img] img->virt_addr[0]=%llx, img->phys_addr[0]=%llx, img->stride[0]=%u;\n",
        img->virt_addr[0], img->phys_addr[0], stride);
...
#ifdef OT_AVP_CVE_V200
        ot_smr_munmap((ot_void *)img->virt_addr[0]);
#endif
        ot_smr_free((ot_phys_addr)img->phys_addr[0]);
    }
...
#else
    ot_smr_flush_cache((ot_phys_addr)data_info->phys_addr, data_info->capacity);
#endif
...
#else
    ot_smr_flush_cache((ot_phys_addr)mem_info->phys_addr, mem_info->size);
#endif
...
ot_s32 ot_avp_cve_img_read_file(ot_avp_cve_img *img, const ot_s8* file_path)
{
    ...
    FILE* fd = fopen(file_path, "rb");
    if (fd == NULL) {
        printf("fopen fail. file not exist, file_path:%s\n", file_path);
        return ret;
    } else {
    ...
    switch (img->type) {
        ...
        case OT_IMAGE_FORMAT_S32C1:
            tmp_u8 = (ot_u8 *)img->virt_addr[0];
            for (h = 0; h < img->height; h++) {
                if (fread(tmp_u8, img->width * sizeof(ot_u32), 1, fd) != 1) {
                    printf("Read file fail\n");
                    goto EXIT;
                }
                tmp_u8 += img->stride[0];
        ...
        case OT_IMAGE_FORMAT_S64C1:
            tmp_u8 = (ot_u8 *)img->virt_addr[0];
            for (h = 0; h < img->height; h++) {
                if (fread(tmp_u8, img->width * sizeof(ot_u64), 1, fd) != 1) {
                    printf("Read file fail\n");
                    goto EXIT;
                }
                tmp_u8 += img->stride[0];
...
#ifndef OT_AVP_CVE_V200
    ot_smr_flush_cache((ot_phys_addr)img->phys_addr[0], (ot_void *)img->virt_addr[0], img->capacity);
#else
    ot_smr_flush_cache((ot_phys_addr)img->phys_addr[0], img->capacity);
#endif
    ret = OT_AVP_CVE_SUCCESS;
...
ot_s32 ot_avp_cve_img_write_file(ot_avp_cve_img *img, const ot_s8* file_path)
{
    ...
    switch (img->type) {
        ...
        case  OT_IMAGE_FORMAT_YVU_420_SEMIPLANAR:
            ...
            break;
        case OT_IMAGE_FORMAT_YUV_422_SEMIPLANAR: //OT_YUV_SEMIPLANAR_422:
        case OT_IMAGE_FORMAT_YVU_422_SEMIPLANAR: //OT_YUV_SEMIPLANAR_422:
            tmp_u8 = (ot_u8 *)img->virt_addr[0];
            tmp_u_u8 = (ot_u8 *)img->virt_addr[1];
            for (h = 0; h < img->height; h++) {
                if (fwrite(tmp_u8, img->width, 1, fd) != 1) {
        ...
        case OT_IMAGE_FORMAT_S8C3_PACKED:
            tmp_u8 = (ot_u8 *)img->virt_addr[0];
            for (h = 0; h < img->height; h++) {
                if (fwrite(tmp_u8, img->width * 3, 1, fd) != 1) {
                    printf("Read file fail\n");
                    goto EXIT;
                }
                tmp_u8 += img->stride[0];
