...
#define SC132GS_ID   132

/****************************************************************************
 * local variables                                                            *
 ****************************************************************************/
#define SC132GS_SHT_min_row 			(0)
#define SC132GS_SHT_max_row_offset 		(8)
#define SC132GS_SHT_max_row_vs 		    (0xb0)
#if 0
#define SC132GS_SHT_DCG_H            	(0x5e88)
#define SC132GS_SHT_DCG_L            	(0x5e89)
#define SC132GS_SHT_VS_H             	(0x5e8c)
#define SC132GS_SHT_VS_L             	(0x5e8d)

#define SC132GS_AGAIN_HCG_H         	(0x5e7c)
#define SC132GS_AGAIN_HCG_L         	(0x5e7d)
#define SC132GS_AGAIN_LCG_H         	(0x5e7e)
#define SC132GS_AGAIN_LCG_L        		(0x5e7f)
#define SC132GS_AGAIN_VS_H          	(0x5e80)
#define SC132GS_AGAIN_VS_L          	(0x5e81)

#define SC132GS_VMAX_H                  (0x320e)
#define SC132GS_VMAX_L                  (0x320f)

#define SC132GS_AWB_B_GAIN              (0x5ac8)
#define SC132GS_AWB_R_GAIN              (0x5ac9)
#endif
...
#define SC132GS_SHT_COARSE_L                       (0x3e02) 
#define SC132GS_SHT_COARSE_M                       (0x3e01) 
#define SC132GS_SHT_COARSE_H                       (0x3e00) 


#define SC132GS_AGAIN_L                         (0x3e09)
#define SC132GS_AGAIN_H                         (0x3e08)
#define SC132GS_DGAIN_L                         (0x3e07)
#define SC132GS_DGAIN_H                         (0x3e06)  
 ...
 * global variables                                                            *
 ****************************************************************************/
#define sc132gs_sensor_set_ctx(pipe, ctx)   ((g_sc132gs_sns_state[pipe]) = (ctx))
#define sc132gs_sensor_reset_ctx(pipe)      (g_sc132gs_sns_state[pipe] = OT_NULL)
...
ot_eis_sns_bus_info g_sc132gs_bus_info[OT_EIS_VI_MAX_PIPE_NUM] = {
        [0] = {
            .com_bus.i2c_dev = 0,
            .bus_addr = SC132GS_I2C_ADDR
        },
...
ot_eis_sns_state *g_sc132gs_sns_state[OT_EIS_VI_MAX_PIPE_NUM] = {OT_NULL};
...
{
    return &g_sc132gs_bus_info[vi_pipe];
}

ot_eis_sns_state *sc132gs_get_ctx(ot_vi_pipe_id vi_pipe)
{
    return g_sc132gs_sns_state[vi_pipe];
}
...
const sc132gs_video_mode_tbl g_sc132gs_mode_tbl[SC132GS_MODE_BUTT]  = {
    {SC132GS_VMAX_1080X1280_30FPS_LINEAR,  SC132GS_FULL_LINES_MAX_LINEAR, 30, 0.5,
        1080, 1280, 0, OT_EIS_WDR_MODE_NONE,  "SC132GS_1080X1280_30FPS_RAW12_LINEAR_MODE"},
};

#define sc132gs_err_mode_print(sns_image_mode, sns_state) \
do { \
    ...
    const ot_eis_sns_state *sns_state)
{
    ...
    ae_sns_dft->full_lines_max = SC132GS_FULL_LINES_MAX;
    ...
    const ot_eis_sns_state *sns_state)
{
    ...
    ae_sns_dft->max_int_time = sns_state->fl_std - SC132GS_SHT_max_row_offset;
    ae_sns_dft->min_int_time = SC132GS_SHT_min_row;
    ae_sns_dft->max_int_time_target = 65535;
    ae_sns_dft->min_int_time_target = SC132GS_SHT_min_row;
...
static ot_s32 cmos_get_ae_default(ot_vi_pipe_id vi_pipe, ot_eis_vi_ae_sensor_default *ae_sns_dft)
{
    ...
    sc132gs_sensor_get_ctx(vi_pipe, sns_state);
...
static ot_void cmos_fps_set(ot_vi_pipe_id vi_pipe, ot_float fps, ot_eis_vi_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = g_sc132gs_mode_tbl[sns_state->img_mode].ver_lines;
    lines_max = g_sc132gs_mode_tbl[sns_state->img_mode].max_ver_lines;
    max_fps = g_sc132gs_mode_tbl[sns_state->img_mode].max_fps;
    min_fps = g_sc132gs_mode_tbl[sns_state->img_mode].min_fps;
...
static ot_s32 cmos_get_vi_alg_default(ot_vi_pipe_id vi_pipe, ot_eis_vi_sns_alg_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = SC132GS_ID;
...
static ot_void cmos_set_pixel_detect(ot_vi_pipe_id vi_pipe, ot_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_EIS_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == SC132GS_1080X1280_30FPS_RAW12_LINEAR_MODE) {
                        full_lines_5fps = (SC132GS_VMAX_1080X1280_30FPS_LINEAR * 30) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }
...
static ot_void cmos_comm_sns_reg_info_init(ot_vi_pipe_id vi_pipe, ot_eis_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].com_bus.i2c_dev = g_sc132gs_bus_info[vi_pipe].com_bus.i2c_dev;
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = g_sc132gs_bus_info[vi_pipe].bus_addr;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = SC132GS_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = SC132GS_DATA_BYTE;
    }
...
static ot_void cmos_linear_sns_reg_info_init(ot_vi_pipe_id vi_pipe, ot_eis_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[0].reg_addr = SC132GS_SHT_COARSE_L;
    sns_state->regs_info[0].i2c_data[1].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[1].reg_addr = SC132GS_SHT_COARSE_M;
    sns_state->regs_info[0].i2c_data[2].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[2].reg_addr = SC132GS_SHT_COARSE_H;
    sns_state->regs_info[0].i2c_data[3].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[3].reg_addr = SC132GS_AGAIN_L;
    sns_state->regs_info[0].i2c_data[4].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[4].reg_addr = SC132GS_AGAIN_H;
    sns_state->regs_info[0].i2c_data[5].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[5].reg_addr = SC132GS_DGAIN_L;
    sns_state->regs_info[0].i2c_data[6].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[6].reg_addr = SC132GS_DGAIN_H;
    sns_state->regs_info[0].i2c_data[7].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[7].reg_addr = SC132GS_VMAX_L;
    sns_state->regs_info[0].i2c_data[8].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[8].reg_addr = SC132GS_VMAX_H;
    ...
    ot_eis_sns_state *sns_state)
{
    switch (sns_image_mode) {
        case SC132GS_1080X1280_30FPS_RAW12_LINEAR_MODE:
            sns_state->fl_std = SC132GS_VMAX_1080X1280_30FPS_LINEAR;
            break;
        default:
            sns_state->fl_std = SC132GS_VMAX_1080X1280_30FPS_LINEAR;
            break;
...
static ot_s32 cmos_set_image_mode(ot_vi_pipe_id vi_pipe, const ot_eis_vi_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < SC132GS_MODE_BUTT; i++) {
        if (sns_image_mode->fps <= g_sc132gs_mode_tbl[i].max_fps &&
            sns_image_mode->width <= g_sc132gs_mode_tbl[i].width &&
            sns_image_mode->height <= g_sc132gs_mode_tbl[i].height &&
            sns_state->wdr_mode == g_sc132gs_mode_tbl[i].wdr_mode) {
            image_mode = (sc132gs_res_mode)i;
            break;
        }
    }

    if (i >= SC132GS_MODE_BUTT) {
        sc132gs_err_mode_print(sns_image_mode, sns_state);
        return OT_FAILURE;
    }
...
static ot_void sensor_global_init(ot_vi_pipe_id vi_pipe)
{
    ...
    sns_state->sync_init = OT_FALSE;
    sns_state->img_mode  = SC132GS_1080X1280_30FPS_RAW12_LINEAR_MODE;
    sns_state->wdr_mode  = OT_EIS_WDR_MODE_BUILT_IN;
    sns_state->fl_std    = SC132GS_VMAX_1080X1280_30FPS_LINEAR;
    sns_state->fl[0]     = SC132GS_VMAX_1080X1280_30FPS_LINEAR;
    sns_state->fl[1]     = SC132GS_VMAX_1080X1280_30FPS_LINEAR;
...
static ot_s32 sensor_ctx_init(ot_vi_pipe_id vi_pipe)
{
    ...
    sc132gs_sensor_get_ctx(vi_pipe, sns_state_ctx);
    ...
    sc132gs_sensor_set_ctx(vi_pipe, sns_state_ctx);
...
static ot_void sensor_ctx_exit(ot_vi_pipe_id vi_pipe)
{
    ...
    sc132gs_sensor_reset_ctx(vi_pipe);

    return;
}

static ot_eis_vi_sns_register_func g_sc132gs_register_to_vi_func = {
    .pfn_sns_init                = sc132gs_init,
    .pfn_sns_exit                = sc132gs_exit,
...
static ot_eis_vi_ae_sensor_register_func g_sc132gs_register_to_ae_func = {
...
static ot_eis_vi_awb_sensor_register_func g_sc132gs_register_to_awb_func = {
...
static ot_s32 sensor_register_callback(ot_vi_pipe_handle pipe_hdl)
{
    ...
    ret = ot_eis_vi_sensor_register_to_vi(pipe_hdl, &g_sc132gs_register_to_vi_func, SC132GS_ID);
    if (ret != OT_SUCCESS) {
        sns_err_trace("sensor register to vi failed!\n");
        goto free_sns_ctx;
    }

    ret = ot_eis_vi_sensor_register_to_ae(pipe_hdl, &g_sc132gs_register_to_ae_func, SC132GS_ID);
    if (ret != OT_SUCCESS) {
        sns_err_trace("sensor register to ae lib failed!\n");
        goto sns_unreg_to_vi;
    }

    ret = ot_eis_vi_sensor_register_to_awb(pipe_hdl, &g_sc132gs_register_to_awb_func, SC132GS_ID);
    if (ret != OT_SUCCESS) {
...
sns_unreg_to_ae:
    ot_eis_vi_sensor_unregister_to_ae(pipe_hdl, SC132GS_ID);
sns_unreg_to_vi:
    ot_eis_vi_sensor_unregister_to_vi(pipe_hdl, SC132GS_ID);
free_sns_ctx:
...
static ot_s32 sensor_unregister_callback(ot_vi_pipe_handle pipe_hdl)
{
    ...
    master_pipe_id = vi_pipe[0];
    ret = ot_eis_vi_sensor_unregister_to_vi(pipe_hdl, SC132GS_ID);
    if (ret != OT_SUCCESS) {
        sns_err_trace("sensor unregister callback function failed!\n");
        return ret;
    }
    ret = ot_eis_vi_sensor_unregister_to_ae(pipe_hdl, SC132GS_ID);
    if (ret != OT_SUCCESS) {
        sns_err_trace("sensor unregister callback function to ae lib failed!\n");
        return ret;
    }

    ret = ot_eis_vi_sensor_unregister_to_awb(pipe_hdl, SC132GS_ID);
    if (ret != OT_SUCCESS) {
...
static ot_s32 sc132gs_set_bus_info(ot_vi_pipe_handle pipe_hdl, ot_eis_sns_bus_info *sns_bus_info)
{
    ot_s32 ret;
    ot_vi_pipe_id vi_pipe[OT_EIS_VI_PIPE_ID_NUM_MAX] = { 0 };
    ot_vi_pipe_id master_pipe_id = 0;
    ...
    master_pipe_id = vi_pipe[0];
    memcpy_s(&g_sc132gs_bus_info[master_pipe_id], sizeof(ot_eis_sns_bus_info), sns_bus_info, sizeof(ot_eis_sns_bus_info));
...
static ot_s32 sc132gs_set_alg_default(ot_vi_pipe_handle pipe_hdl, ot_eis_vi_sns_alg_default *sns_alg_def)
{
    ot_s32 ret;
    ot_vi_pipe_id vi_pipe[OT_EIS_VI_PIPE_ID_NUM_MAX] = { 0 };
    ot_vi_pipe_id master_pipe_id = 0;
...
static ot_s32 sc132gs_set_awb_default(ot_vi_pipe_handle pipe_hdl, ot_eis_vi_awb_sensor_default *sns_awb_def)
{
    ot_s32 ret;
    ot_vi_pipe_id vi_pipe[OT_EIS_VI_PIPE_ID_NUM_MAX] = { 0 };
    ot_vi_pipe_id master_pipe_id = 0;
...
static ot_s32 sc132gs_set_reg_table(ot_vi_pipe_handle pipe_hdl, ot_eis_sns_reg_table *reg_table, ot_s32 table_cnt)
{
    ot_s32 ret;
    ot_vi_pipe_id vi_pipe[OT_EIS_VI_PIPE_ID_NUM_MAX] = { 0 };
    ot_vi_pipe_id master_pipe_id = 0;
...
ot_eis_sns_obj g_sns_sc132gs_obj = {
    .pfn_sns_register              = sensor_register_callback,
    .pfn_sns_unregister            = sensor_unregister_callback,
    .pfn_sns_standby               = sc132gs_standby,
    .pfn_sns_restart               = sc132gs_restart,
    .pfn_sns_set_mirror_flip       = OT_NULL,
    .pfn_sns_set_blc_clamp         = sc132gs_blc_clamp,
    .pfn_sns_set_bus_info          = sc132gs_set_bus_info,
    .pfn_sns_set_init              = sensor_set_init,
    .pfn_sns_set_alg_default       = sc132gs_set_alg_default,
    .pfn_sns_set_awb_default       = sc132gs_set_awb_default,
    .pfn_sns_set_reg_table         = sc132gs_set_reg_table,
};
