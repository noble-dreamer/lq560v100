...
typedef struct {
	ot_s32      sensor_mask;
	ot_u16 timestamp_fsync;
	ot_s16  accel[3];
	ot_s16  gyro[3];
	ot_s16  temperature;
	ot_s8   accel_high_res[3];
	ot_s8   gyro_high_res[3];
...
struct inv_imu_device {
	 ...
	 */
	void (*sensor_event_cb)(ot_u16 packet_count, ot_u16 idx, ot_u64 time_inter, inv_imu_sensor_event_t *event);

	ot_u8               fifo_data[FIFO_MIRRORING_SIZE]; /**< FIFO mirroring memory area */
	ot_u8               dmp_is_on; /**< DMP started status */
	ot_u8               endianness_data; /**< Data endianness configuration */
	ot_u8               fifo_highres_enabled; /**< Highres mode configuration */
	INV_IMU_FIFO_CONFIG_t fifo_is_used; /**< FIFO configuration */
	ot_u64              gyro_start_time_us; /**< Gyro start time used to discard first samples */
	ot_u64 accel_start_time_us; /**< Accel start time used to discard first samples */
};
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_enable_accel_low_power_mode(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_disable_accel(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_disable_gyro(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_enable_fsync(struct inv_imu_device *s);
 ...
 *  @return                            0 on success, negative value on error.
 */
ot_s32 inv_imu_set_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
ot_s32 inv_imu_get_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
ot_s32 inv_imu_set_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
ot_s32 inv_imu_get_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_get_data_from_registers(struct inv_imu_device *s);
 ...
 */
ot_s32 inv_imu_get_data_from_fifo(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_enable_high_resolution_fifo(struct inv_imu_device *s);

/** @brief Disable 20 bits raw acc and raw gyr data in fifo.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
ot_s32 inv_imu_disable_high_resolution_fifo(struct inv_imu_device *s);
 ...
 */
ot_u32 inv_imu_get_timestamp_resolution_us(struct inv_imu_device *s);
 ...
 *  @return                0 on success, negative value on error.
 */
ot_s32 inv_imu_set_endianness(struct inv_imu_device *s, INTF_CONFIG0_DATA_ENDIAN_t endianness);
 ...
 *  @return                0 on success, negative value on error.
 */
ot_s32 inv_imu_configure_fifo_data_rate(struct inv_imu_device *s, FDR_CONFIG_FDR_SEL_t dec_factor);

/** @brief Return driver version x.y.z-suffix as a ot_char array
 *  @return driver version a ot_char array "x.y.z-suffix"
 */
const ot_char *inv_imu_get_version(void);
