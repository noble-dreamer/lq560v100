...
#define OT_MFUSION_QUATERNION_MAX_NUM 128
typedef struct {
    /*
...
typedef struct {
     ...
     */
    ot_s32 imu_lut[OT_MFUSION_TEMPERATURE_LUT_SAMPLES][OT_MFUSION_AXIS_NUM];
    ot_s32 gyro_lut_status[OT_MFUSION_TEMPERATURE_LUT_SAMPLES][OT_MFUSION_LUT_STATUS_NUM];
...
typedef struct {
    ...
    union {
         ...
         */
        ot_s32 temperature_matrix[OT_MFUSION_MATRIX_TEMPERATURE_NUM];
        /*
...
typedef struct {
    ot_bool enable;
    ot_s32 drift[OT_MFUSION_AXIS_NUM];
} ot_mfusion_drift;

typedef struct {
    ot_bool enable;
    ot_s32 matrix[OT_MFUSION_MATRIX_NUM];
} ot_mfusion_six_side_calibration;

typedef struct {
    ot_bool enable;
    ot_u32 crop_ratio;       /* RW; Range:[20,100]; Crop ratio of output image. */
} ot_mfusion_rotation_compensation;

typedef struct {
    ot_u32 buf_size;
} ot_mfusion_rotation_cfg;
...
typedef struct {
    ...
    ot_s32 z;
    ot_u64 pts;
} ot_mfusion_quaternion_data;

typedef struct {
    ot_u32 num;
    ot_mfusion_quaternion_data data[OT_MFUSION_QUATERNION_MAX_NUM];
} ot_mfusion_quaternion_buf;
