...
#define CYTTSP5_TOUCHLOG_ENABLE 1
		...
		for (j = 0; j <= CY_ABS_W_OST; j++) {
			...
			if (sig != CY_IGNORE_VALUE) {
				input_report_abs(md->input, sig,
					tch->abs[CY_TCH_X + j]);
					// printk("sig 0x%x value %d CY_NUM_ABS_SET %d \r\n", sig, tch->abs[CY_TCH_X + j], CY_NUM_ABS_SET);
			}
		}
...
int cyttsp5_mt_probe(struct device *dev)
{
	...
	struct cyttsp5_mt_platform_data *mt_pdata = _cyttsp5_platform_data.mt_pdata;
	int rc = 0;
