...
static int setup_report_descriptor(struct cyttsp5_core_data *cd)
{
	...
	for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
		...
		if (field) {
			...
			dev_vdbg(cd->dev, "%s: ofs:%ld size:%ld min:%ld max:%ld bofs:%ld report:%d",
...
#define CONFIG_PM_RUNTIME 0 // add by jialei for build
...
{
	struct cyttsp5_core_data *cd;
	struct cyttsp5_platform_data *pdata = &_cyttsp5_platform_data;// dev_get_platdata(dev);
	...
	// if irq_num from module_param, use it
	if (irq != pdata->core_pdata->irq_gpio) {
		pdata->core_pdata->irq_gpio = irq;
		dev_info(dev,
			"%s: INIT CYTTSP IRQ gpio=%d\n", __func__, pdata->core_pdata->irq_gpio);
	}
