...
- 矫正边缘/无有效视差区域：模型在该处没有真实对应点，代价体取到搜索范围边界，视差**饱和到最大候选 128px（实测可达 ~137px）**；前端 LUT 把 `disp≥~118px` 画红、`≥128px` 顶到最红，所以远景边缘会误显成红色块。**板端已在传输前把 `Q5≥4096`（disp≥128px）掩膜为 0（无效）**，主机端「q5<0.1→黑色」渲染与框中心距离（q5<16 跳过）自动生效。
